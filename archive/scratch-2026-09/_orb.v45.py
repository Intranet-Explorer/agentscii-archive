...
# v45 FIX (raze): the flagged socket-shadow region (rows 13-21, cols 4-24) is EMPTY
# background -- fill it with a DENSE directional light->dark gradient that bridges from
# the lit sclera down to the void. Deterministic + directional (follows upper-left source).
ramp_7_0 = shade_ramp(7, 0, 5)         # light gray -> black
for py in range(ph):
    for px in range(W):
        cr = py // 2
        if not (13 <= cr <= 21 and px < 24): continue             # the flagged region
        if cv.get_pixel(px,py) != 0: continue                     # only fill empty cells
              # distance from the upper-left light source, normalized
    ...
    for px in range(W):
        ...
        j=hsh(px,cr,13)
           ...
           # (a) PUPIL: SOLID TRUE-BLACK VOID + THIN GRAY RIM ONLY AT THE EDGE.
            ...
            # pupil/iris boundary so the void reads as depth, not a flat block.
        if d < pupil_r + 1.5:
            t_pup = d / max(0.5, pupil_r + 1.5)
                 # v45 FIX (raze): gate flagged 62+ IDENTICAL pure-black cells as flat fill.
                 # A lit pupil has DEPTH -- a SOLID-COLOR radial gradient (black center ->
                 # dark-gray mid -> light-gray rim), NOT density dithering (v43 mottling)
                 # and NOT a thin invisible rim (v44). The gradient must be VISIBLE across
                 # the flagged region. Plus an upper-left catchlight glint.
            if t_pup < 0.35:
                cv.glyph_override[(cr,px)] = ('\u2588', 0, 0)       # black void center
            elif t_pup < 0.60:
                cv.glyph_override[(cr,px)] = ('\u2588', 8, 0)       # dark-gray depth ring
            elif t_pup < 0.82:
                cv.glyph_override[(cr,px)] = ('\u2588', 7, 0)       # light-gray mid-ring
            else:
                cv.glyph_override[(cr,px)] = ('\u2588', 15, 0)      # bright rim at the edge
                 # upper-left catchlight: the source catches the top-left of the void surface
            cd = math.hypot(px - (iris_cx - 3), (cr*2+0.5) - (iris_cy - 4))
            if cd < 2.5 and d < pupil_r * 0.85:
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v45.ans","w").write(out)
