...
def hooded_figure(cv, fx, feet_y, height=14.0, lean=0.0):
    """A lone HOODED traveler as a small silhouette on the band -- v1.2's robe, ported in
    proportion: one WIDE cylindrical-shaded cloak trapezoid (narrow at shoulders ~2, flaring to a
    wide hem ~5) + a distinct shaded hood cap + a dark face-void. The scroll regressed this into a
    thin pillar (half_sh 1.6 -> half_hem 3.0, no cylindrical term); that reads as a monolith, not a
    robe, so we restore the flare + the rounded cross-section. `lean` tilts the body toward the light
    so panel II reads as WALKING TOWARD it rather than standing straight (panel I)."""
    ...
    half_hem, half_sh = 5.0, 2.0                   # v1.2 flare: ~2 wide at shoulders -> ~5 at hem
    for y in range(int(sh_y), int(hem_y) + 1):
        ...
        for x in range(int(fx - hw), int(fx + hw) + 1):
            if not cv.in_bounds(x, y):
                continue
            cyl = 1.0 - 0.4 * (abs(x - fx) / max(1e-6, hw))   # rounded cross-section -> 3D robe
