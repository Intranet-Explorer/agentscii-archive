...
# THE WATCHER (_orb v36) -- raze + hollis. Built on v35 (flat-region gate still blocking).
# TWO TARGETED FIXES over v35:
#         (1) UPPER-LEFT SOCKET (rows 13-21 pixel, cols 4-24): make shade_ramp gradient cover
#             EVERY cell in the region, not just some. The flat region was being created by
#             earlier passes setting pixels to 8, then PASS 7d only processed pixels already=8
#             but didn't cover the whole area.
#         (2) LOWER-RIGHT REGION (rows 33-44 pixel, cols 52-73): new flat region from the glow
#             ring fading into darkness. Add a directional gradient there too.
W=80; H=52
...
# --- PASS 7b: LOWER-RIGHT SHADOW GRADIENT -- directional gradient via shade_ramp
ramp_8_0_lr = shade_ramp(8, 0, 5)
for py in range(ph):
    for px in range(W):
        ...
        if dx > 5 and dy > 5:
            ...
            if d < 50:
                cr = py // 2
                base = 1.0 - d/50.0
                 # Position-based gradient: top-left of this region is lighter, bottom-right darker
                t = (px + (cr - 16) * 3) / 40.0
                idx = min(4, int(t * 4))
                glyph, fg, bg = ramp_8_0_lr[idx]
                cv.set_pixel(px, cr*2, fg)
                cv.set_pixel(px, cr*2+1, bg)
                if glyph != '\u2588' and glyph != ' ':
                    cv.glyph_override[(cr, px)] = glyph
...
# --- PASS 7d: UPPER-LEFT SOCKET TEXTURE -- directional gradient via shade_ramp, COVER EVERY CELL
ramp_7_0_socket = shade_ramp(7, 0, 5)
for py in range(ph):
    for px in range(W):
        cr = py // 2
         # Upper-left socket area: pixel rows 13-21 = cell rows 6-10, cols 4-24
        if 6 <= cr <= 10 and 4 <= px <= 24:
...
data = "\n".join(out) + "\x1b[0m\n"
with open("_orb.v36.ans","w",encoding="utf-8") as f:
    f.write(data)
print("wrote _orb.v36.ans:", len(data), "bytes")
