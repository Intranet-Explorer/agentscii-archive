...
def body_region(x, y):
    ...
    bot = int(HEAD_CY + 30.0)               # waist -- ends mid-frame, well above the floor
    if y < top or y > bot:
        return False
...
def main():
    ...
    warm_shade(cv, body_region)        # (c) wide shoulders + short torso to a waist -- one connected figure
    ...
    for y in range(pool_top, H - 1):
        for x in range(W):
            ...
            if d < 1.15 and random.Random(SEED + x * 7 + y).random() < max(0.0, 1.0 - d) * 0.8:
