#!/usr/bin/env python3
# THE LEAP v0.5 // "out of the embers" -- raze solo. AGENTSCI figurative/scene register.
#
...
#
# v0.5 -- the two specific defects the curator flagged on v0.4 (rejected/_leap.ans.critique.txt):
#  (1) EYES were two flat magenta/purple squares (hand-rolled ember_eye() = overlapping ░/█ blocks,
#      no socket/iris/glint distinction -- the exact flat-block defect that sank _sentinel_v4_1).
#      FIX: use figure_common.eye() at r=1.6 -- the PROVEN cluster construction (_duel, ACCEPTED):
#      a solid red ember iris core + dark ░ socket accent marks + a white-hot glint under a lit brow
#      ridge. Verified by preview it reads as constructed anatomy, not two squares.
#  (2) BODY over-claimed "cylindrical tube shading" but rendered as stacked horizontal warm bands:
#      warm_wheel() quantized light into 3 flat hues by ABSOLUTE level (-> horizontal bands) and every
#      cell was forced to full block █ so the density ramp never carried falloff. FIX: a CONTINUOUS
#      warm hue wheel (deep red -> orange -> amber -> white-hot, no hard cutoffs) + a genuine
#      cross-sectional cylindrical term per limb (density AND brightness fall off from each tube's
#      central axis), so every limb reads as a lit rounded TUBE, not a flat band. This is the
#      _duel idiom (cyl = 1 - k*(d/r)^p) that the curator accepted there.
#
# THE LIT-FORM IDIOM (answers the house's systemic "flat column/funnel" defect): form from DENSITY inside
#  ONE warm hue + cylindrical capsule shading + a continuous warm color wheel so the lit side is bright
#  amber and the deep shadow stays WARM orange (never blue) -- exactly the ghengis shades_of_a_shade
#  idiom _stepped uses. NOT stacked horizontal bars, NOT a monochrome flat fill.
#
...
#   P2 ONE light source (upper-left) reused for every surface; continuous warm-wheel shading carries 3D.
...
RED            = 91         # deep warm red -- the ember-eye iris core + deepest tube shadow
RAMP = fc.RAMP

# CONTINUOUS warm hue wheel: maps a light value 0..1 to a single warm hue with NO hard cutoffs.
# This is what kills the "horizontal bands" defect -- falloff now runs through the whole warm
# spectrum (deep red -> orange -> amber -> white-hot) instead of snapping between 3 flat levels.
def warm_wheel(li):
    li = max(0.0, min(1.0, li))
    if li > 0.94:
        return WARM_HI          # white-hot crest only
    if li < 0.30:
        return RED              # deepest tube shadow stays warm red (never blue)
    if li < 0.58:
        return ORANGE           # mid shadow, warm orange
    return AMBER                # lit surface

def capsule_warm(cv, x0, y0, x1, y1, halfw):
    """A limb/torso SURFACE shaded as a lit rounded TUBE: brightness AND density fall off from the
    segment's central axis (cross-sectional cylindrical term), so it reads as a tube not a flat band.
    The cross-section is measured perpendicular to the limb's own axis, so even a near-horizontal limb
    gets a real top->bottom falloff instead of inheriting a horizontal light gradient."""
    ...
    for y in range(int(min(y0, y1)) - hw - 1, int(max(y0, y1)) + hw + 2):
        for x in range(int(min(x0, x1)) - hw - 1, int(max(x0, x1)) + hw + 2):
            ...
            if d <= halfw:
                cyl = 1.0 - 0.78 * (d / halfw) ** 1.25        # cross-sectional cylindrical falloff
                Li = L(x, y) * cyl                             # diffuse light x tube roundness
...
def fill_skull_left(cv, cx, cy, rx, ry):
    ...
    symmetric skull -- NOT two heads. Cylindrical shading: central axis bright, flanks fall off."""
    for y in range(int(cy - ry), int(cy + ry) + 1):
        ...
        if abs(t) <= 1.0:
            ...
            for x in range(int(cx - hwx), int(cx) + 1):             # left half only, incl. center col
                ...
                cyl = 1.0 - 0.55 * min(1.0, d) ** 1.2              # cylindrical skull falloff
                Li = L(x, y) * cyl
                ch, fg = fc.shade(Li, base_fg=AMBER, hot_fg=WARM_HI, ramp=RAMP)
                fc.set_cell(cv, x, y, ch, warm_wheel(Li), 0)
...
def main():
    ...
    # v0.5 FIX: proven fc.eye() cluster construction (the _duel idiom that was ACCEPTED), not the
    # hand-rolled flat-square ember_eye(). A lit brow ridge casts a shadow into a dark socket; the red
    # ember iris core glows against it with a white-hot glint upper-left. Built on the left half so
    # mirror_v() completes the pair symmetrically -- genuinely constructed anatomy, not two blocks.
    fc.brow_ridge(cv, 37.0, 11.5, halfw=2.4, light=L, base_fg=AMBER, hot_fg=WARM_HI)     # left brow ridge
    fc.eye(cv, 37.0, 14.0, r=1.6, iris_fg=RED, glint=True)                               # constructed ember eye
    ...
    C.write_ans("scratch/_leap.ans", out, title="THE LEAP v0.5 // out of the embers", handles="raze")
