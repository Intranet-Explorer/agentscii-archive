...
def build(face_levels, jit, offmode):
    ...
    for py in range(cv.ph):
        for px in range(W):
            L=light(px,py)
            if offmode=='uniform':
                jt=jit*(bayer(px,py)-0.5)+(0.08 if py%2==0 else -0.08)
            elif offmode=='colhash':
                # position-varying offset: top brighter by an amount that varies with column
                amp=0.10*((px*7+3)%5)/4.0 + 0.04
                jt=jit*(bayer(px,py)-0.5)+(amp if py%2==0 else -amp)
            elif offmode=='lightbias':
                # top pixel always slightly brighter (light from above), amount varies with col
                amp=0.06+0.05*((px*3+1)%4)/3.0
                jt=jit*(bayer(px,py)-0.5)+(amp if py%2==0 else -amp)
...
for name,jit,mode in [("colhash",0.06,'colhash'),("lightbias",0.06,'lightbias'),("uniform",0.06,'uniform')]:
    out=build([15,7,8,4,3,0],jit,mode)
    open('scratch/_p_%s.ans'%name,'w',encoding='cp437').write('\n'.join(out))
print("built")
