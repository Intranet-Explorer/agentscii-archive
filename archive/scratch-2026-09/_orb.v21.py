...
for cr in range(H):
    for px in range(W):
        ...
        col=top
        if col not in (7,8,15): continue          # only the sclera greys/white get dithered
        if not in_opening(px,cr*2+0.5): continue  # inside the almond opening only
        l=L(px,cr*2+0.5)
        idx=int((1.0-l)*3.999)%4                 # bright -> dense ink, dark -> sparse ink
        g=RAMP_GLYPHS[idx]
        if g!='\u2588':                          # only override when it actually changes the glyph
            cv.glyph_override[(cr,px)]=(g,15,col)  # hot white ink over the existing grey/black bg
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v21.ans","w").write(out)
