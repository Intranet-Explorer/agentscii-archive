...
# v44 FIX (raze): the flagged socket-shadow region (rows 13-21, cols 4-24) is EMPTY (col 0)
# background, not col 8 -- so a !=8 guard skips it. FILL it with a directional light->dark
# gradient that bridges from the lit sclera down to the void below: real ACiD socket shading,
# deterministic + directional (follows the upper-left source), NOT random scatter.
ramp_7_0 = shade_ramp(7, 0, 5)        # light gray -> black
for py in range(ph):
    for px in range(W):
        cr = py // 2
        if not (13 <= cr <= 21 and px < 24): continue            # the flagged socket-shadow region
        if cv.get_pixel(px,py) != 0: continue                    # only fill genuinely empty cells
             # distance from the upper-left light source, normalized across the region
        t = math.hypot(px - lx, py - ly) / 60.0
        t = max(0.0, min(1.0, t))
        idx = min(4, int(t * 4 + 0.5))
        glyph, fg, bg = ramp_7_0[idx]
        if glyph == '\u2588':
    ...
    for px in range(W):
        ...
        if d < pupil_r + 1.0:
            t_pup = d / max(0.5, pupil_r)
                # v44 FIX (raze): gate flagged 62 IDENTICAL pure-black cells as flat fill.
                # A lit pupil has DEPTH -- but via a SOLID-COLOR radial gradient (black center
                # -> dark-gray rim), NOT density dithering. Dithered ░▒▓ across the core = the
                # mottled-void defect that sank v29/v30; solid-color ramp shades across itself
                # cleanly. Plus an upper-left catchlight catching the curved surface.
            if t_pup < 0.55:
                cv.glyph_override[(cr,px)] = ('\u2588', 0, 0)      # clean black void center
            elif t_pup < 0.78:
                cv.glyph_override[(cr,px)] = ('\u2588', 8, 0)      # dark-gray depth ring
            else:
                cv.glyph_override[(cr,px)] = ('\u2588', 7, 0)      # light-gray rim at the edge
                # upper-left catchlight: the source catches the top-left of the void surface
            cd = math.hypot(px - (iris_cx - 3), (cr*2+0.5) - (iris_cy - 4))
            if cd < 2.2 and d < pupil_r * 0.8:
                cv.glyph_override[(cr,px)] = ('\u2588', 15, 0)     # bright catchlight glint
            continue
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v44.ans","w").write(out)
