...
def galaxy_density(r, c):
    ...
    arm1 = max(0.0, 1.0 - abs(((theta + 0.0) * d / (2 * math.pi * b)) % 1.0 - 0.5) * 2.2)
    arm2 = max(0.0, 1.0 - abs(((theta + math.pi) * d / (2 * math.pi * b)) % 1.0 - 0.5) * 2.2)
    arms = max(arm1, arm2) * max(0.0, 1.0 - d / 13.0) ** 1.4
        ...
        for c in range(W):
            ...
            if st is not None:
                ...
                cells.append(sgr(fg, 40) + g)
                  ...
                  # distant spiral galaxy -- warm core (amber/yellow), cool arms (cyan/blue).
                di = int(gdens * 3.999)
                g = RAMP[di]
                  # core reads hot (amber/yellow), arms read cool (cyan/blue) by distance from center
...
emit(sgr(96, 40) + "NEBULA v1.1 -- nebula + starfield + spiral galaxy cluster, joint pass")
emit(RESET)
