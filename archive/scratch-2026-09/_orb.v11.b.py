...
#   NCRYPTS 8->14, spiral term d*0.14 -> d*0.05 (fibers run radially), dark valley l*=0.26 -> 0.18
...
for py in range(ph):
    for px in range(W):
        ...
        if d<iris_r:
            ...
            l=base_l
            band = math.sin(ang*NCRYPTS + d*0.05)
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v11.b.ans","w").write(out)
