import re
#!/usr/bin/env python3
# make_poster_v7.py -- raze build. POSTER v7.0 capstone, clean rebuild after the v3.2 REJECT.
#
# The critique (rejected/hollis-raze-poster.ans.critique.txt) was specific and fair:
#   KEEP: SGR hygiene, reused 5x7 AGENTSCI wordmark treatment top+bottom, double-line frame,
#         open/close credit cards, city silhouette base.
#   FIX  (the one real failure -- COMPOSITION):
#     (1) CORE must READ as a solid glowing reactor SPHERE at the visual center -- dense
#         concentric cycling rings filling every cell inside the radius, white-hot core,
#         a thin bright rim to isolate it. NO clear/breathing ring (that fragmented v3.2).
#     (2) The MIDDLE must not be mostly black. A dense full-bleed color-cycling wash fills
#         EVERY background cell so the three motifs cohere OVER a saturated field, not over
#         empty space. Negative space only where there's a strong surrounding field to pop against.
#     (3) Hierarchy: wordmark title card (top) -> orb (center, dominant) -> city (base),
#         rain receding behind. Make the render match that.
#
# This is a clean rebuild of the composition layer; everything verified-good from v3.2/v6
# (frame, wordmark FONT + treatment, credit card, SGR c() hygiene) is carried over unchanged.
...
def c(fg, bg=0):
    # raw SGR passthrough: a value already in the 30-37 / 90-97 range is emitted verbatim
    # (lets us place TRUE white 97 and bright cyan 104 directly for the core luminance pass);
    # otherwise map the legacy 0-7 / 8-15 index through the standard ANSI offset.
...
# ---------------------------------------------------------------------------
# 1. BACKGROUND: DENSE full-bleed color-cycling wash -- EVERY cell lit (no black voids).
#    A slow diagonal phase so the field reads as a living, shifting bed the motifs cohere over.
#    Dithered ░/▒/▓ mix so it's texture, not flat fill (house: block-density dithering).
# ---------------------------------------------------------------------------
for y in range(H_INNER):
    for x in range(INNER_W):
        ...
        col = hue(ph)
        m = (x*3 + y*5) % 4
        ch = '█' if m < 3 else '▓'              # heavy blocks -> saturated field, no near-black voids
        set_cell(x, y, ch, col, 0)

# ---------------------------------------------------------------------------
# 2. RADIAL GLOW behind the orb -- a saturated halo so the reactor reads as emitting
#    light into the field (the move accepted CORE used).
# ---------------------------------------------------------------------------
CCX, CCY = CX, 13            # true vertical center of the inner field -> orb leads the eye
RX, RY = 16.0, 10.0
for y in range(H_INNER):
    for x in range(INNER_W):
...
#    white-hot center, a thin bright rim to isolate it. NO clear ring. Bigger + brighter so it
#    dominates the field and leads the eye at true vertical center.
# ---------------------------------------------------------------------------
for y in range(H_INNER):
    for x in range(INNER_W):
        dx=(x-CCX)/RX; dy=(y-CCY)/RY
        ...
        if r < 0.30:
            set_cell(x, y, '█', 97, 0)           # TRUE white-hot core -> brightest cell in the file
        elif r < 0.52:
            ch = '█' if (x+y)%2==0 else '▓'
            set_cell(x, y, ch, 104, 0)           # bright-cyan hot halo
        elif r < 0.80:
            ch = '▓' if (x+y)%2==0 else '▒'
            set_cell(x, y, ch, col, 0)           # colored cycling body
        else:
            ch = '▒' if (x+y)%2==0 else '░'
            set_cell(x, y, ch, col, 0)
...
for y in range(H_INNER):
    for x in range(INNER_W):
        ...
        r=math.sqrt(dx*dx+dy*dy)
        if 0.97 < r <= 1.06:
            set_cell(x, y, '█', 104, 0)          # crisp bright-cyan rim -> coherent circle edge

# ---------------------------------------------------------------------------
# 6. HERO: AGENTSCI wordmark -- REUSED house mark (5x7 block letters on a dark interior band),
#    the pack01 LOGO treatment. Title card at top; a matching credit card closes the piece.
# ---------------------------------------------------------------------------
FONT = {
...
for y in range(band_top, min(band_bot, H_INNER)):
    for x in range(start_cx-2, start_cx+total_w+3):
        set_cell(x, y, ' ', 0, 0)               # black interior band (matches closing card)
        ...
        for rx,bit in enumerate(row):
            ...
            for dx,dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                ...
                if 0<=nx<INNER_W and 0<=ny<H_INNER:
                    set_cell(nx, ny, '▒', 9, 0)         # dim cyan halo on black (matches closing card)
...
# ---------------------------------------------------------------------------
# Render to ANSI (house frame + dithered inner side band + reset tail).
# ---------------------------------------------------------------------------
...
lines.append(center("POSTER v8.0 -- pack capstone", c(9,0)))
lines.append(c(15,0) + "\u255a" + c(14,0) + "\u2550"*INNER_W + c(15,0) + "\u255d")
lines.append("\x1b[0m")

out = "\n".join(lines)
with open("scratch/hollis-raze-poster.v8.ans", "w", encoding="cp437") as f:
    f.write(out)
# hygiene gate (pack04 lesson): cp437 on disk, zero utf8-box bytes, SGR 0-107
_b = out.encode("cp437")
_utf8box = _b.count(b"\xe2\x94") + _b.count(b"\xe2\x96")
_sgrbad = [int(p) for s in re.findall(r"\x1b\[([0-9;]*)m", out) for p in (s.split(";") if s else ["0"]) if not 0<=int(p)<=107]
print(f"HYGIENE cp437_ok=True utf8-box={_utf8box} badSGR={_sgrbad[:3]} bytes={len(_b)}")

# quick self-checks
import re
sgroups = set(re.findall(r"\x1b\[(\d+);\d+m", out))
print("lines:", len(lines), "inner:", H_INNER, "cols max:", max(len(l.replace('\x1b[0m','')) for l in lines))
print("SGR fg codes:", sorted(int(s) for s in sgroups))
