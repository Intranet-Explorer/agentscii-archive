...
# ---- PASS 2: calm sea -- v7: genuine horizontal RIPPLE LINES, not a confetti scatter.
# Each ripple row is a few short HORIZONTAL runs of block-density ticks that undulate (a gentle
# sine wave in x), so the water reads as moving surface, not static. Density + brightness both
# grow toward the bottom (nearer = more visible) and thin out near the horizon (far = faint).
import math as _m
SEA_TOP = HORIZON
SEA_BOT = H - 1
for wy in range(SEA_TOP + 1, SEA_BOT, 2):                 # one ripple row every 2 rows
    depth = (wy - HORIZON) / max(1.0, (H - 1 - HORIZON))   # 0 at horizon -> 1 at bottom
    fg = SEA if wy > HORIZON + 6 else SEA_RIP              # bluer nearer the bottom, greyer up top
    amp = 2.0 * depth                                      # wave amplitude grows with depth (calm far, rolling near)
    period = 11 - int(4 * depth)                           # shorter wavelength near the viewer
    phase = (wy * 3.7) % W
    # density: faint & sparse near horizon, a touch denser near bottom -- but never a full grid
    dash_len = 2 + int(2 * depth)                          # short runs
    gap      = 4 - int(depth)                              # gaps close up slightly with depth
    x = int(phase) % W
    while x < W:
        # gentle vertical undulation so the run isn't a flat bar; skip some cells to keep it loose
        if (x + wy) % 3 != 0:
            ramp_idx = 2 if depth < 0.45 else 1             # dimmer far, slightly denser near
            cv.set(x, wy, C.RAMP[ramp_idx], fg, 0)
        x += dash_len
        x += max(1, gap)                                   # a calm gap between ripple runs
# ---- PASS 2b: faint amber reflection of the lamp in the water (ties the single accent into the sea)
lamp_x = 8                                                   # under the near-end lamp post (DECK_X0+2)
for wy in range(23, min(H - 1, 30), 1):                        # just below the deck band
    wob = int(_m.sin(wy * 1.3) * 2)                        # wobbly vertical shimmer under the lamp
    if (wy % 2 == 0):
        cv.set(lamp_x + wob, wy, C.RAMP[2], LAMP, 0)       # dim amber tick, broken up = reflection
...
out = []
cv.render(out)
C.write_ans("scratch/_wharf.ans", out, title="LOW TIDE // the wharf at dusk v7.0",
            handles="raze")
