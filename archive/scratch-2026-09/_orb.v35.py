...
# THE WATCHER (_orb v35) -- raze + hollis. Built on v34 (flat-region gate passed but pupil
# core still reads mottled from thick multi-density dither band). ONE TARGETED FIX:
#         PUPIL CORE: solid black center (the void), ONE coherent thin gray rim with uniform
#         50% density (▒) -- no hatched patchwork. Directionality only at the very outer edge
#         via color shift, not glyph mixing. Clean and readable as an eye.
W=80; H=52
...
# The void reads as a hole because the center is solid black. The rim is ONE uniform
# density level (▒ = 50%) so it's a smooth gray edge, not a hatched patchwork.
# Directionality only at the very outer edge via color shift (7 vs 8), not glyph mixing.
for py in range(ph):
    for px in range(W):
        dx=px-iris_cx; dy=py-iris_cy; d=math.hypot(dx,dy)
        if d < pupil_r + 1.0:
            ...
            if t < 0.72:
                cv.set_pixel(px,py, 0)        # solid black void center
            elif t < 0.92:
                cv.set_pixel(px, cr*2, 0)     # fg black
                cv.set_pixel(px, cr*2+1, 8)   # bg gray -- uniform dither = clean rim
                cv.glyph_override[(cr, px)] = '\u2592'    # 50% density, coherent
            else:
...
data = "\n".join(out) + "\x1b[0m\n"
with open("_orb.v35.ans","w",encoding="utf-8") as f:
    f.write(data)
print("wrote _orb.v35.ans:", len(data), "bytes")
