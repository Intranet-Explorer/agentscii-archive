...
def frost_spokes():
    ...
    def crack(ang, r0, reach, wander, phase, ch_near, fg_near, ch_far, fg_far, skip, dirsign=1):
        for r in range(r0, int(reach)):
            ...
            if abs(r - int(reach * 0.72)) <= 1:
                bang = ang + random.uniform(0.35, 0.7) * dirsign
    ...
    for k in range(n):
        ...
        if k % 3 == 0:                                               # DOMINANT white lines from just past the fins outward
            crack(ang, 19, reach, wander, phase, "\u2580", 15, "\u2580", 14, skip=0.05, dirsign=(1 if k % 2 else -1))
        elif k % 3 == 1:                                             # SECONDARY cyan network between the mains
            crack(ang, 21, reach * 0.8, wander, phase, "\u2591", 14, "\u2591", 6, skip=0.14, dirsign=(1 if k % 2 else -1))
        else:                                                        # HAIR -- fine dim-blue fringe far out only
            crack(ang + 0.13, 23, reach * 0.7, wander * 1.3, phase, "\u2580", 6, "\u2591", 6, skip=0.28, dirsign=(1 if k % 2 else -1))
...
# Runs AFTER every left-half-only pass (vessel/bloom/frost lattice) so all of them get copied.
# P4a's full-width texture_fill is symmetric, so re-mirroring it is a harmless no-op.
mirror(cv, axis='v')
