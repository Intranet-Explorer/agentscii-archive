    ...
    for px in range(W):
        ...
        if d < pupil_r + 1.0:
            t_pup = d / max(0.5, pupil_r)
                # v43 FIX (raze): gate flagged 62 IDENTICAL pure-black cells as a flat fill.
                # A real lit pupil has DEPTH -- deterministic radial+directional gradient via
                # shade_ramp(), NOT random per-cell scatter (the v29/v30 mottling defect). Black
                # center -> gray rim, plus an upper-left catchlight catching the curved surface.
            ramp_0_8 = shade_ramp(0, 8, 5)        # black core -> gray rim
            idx = min(4, int(t_pup * 4 + 0.5))
            glyph, fg, bg = ramp_0_8[idx]
            if glyph == '\u2588':
                cv.glyph_override[(cr,px)] = ('\u2588', fg, 0)
            else:
                cv.glyph_override[(cr,px)] = (glyph, fg, bg)
                # upper-left catchlight: the light source catches the top-left of the void
            cd = math.hypot(px - (iris_cx - 3), (cr*2+0.5) - (iris_cy - 4))
            if cd < 2.0 and d < pupil_r * 0.7:
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v43.ans","w").write(out)
