...
# --- PASS 10: ANATOMICAL FOREGROUNDS (v28, raze+hollis). The half-block packer collapses any cell
#     whose top==bottom pixel to a plain space+bg=0, which the distinct_colors_in_subject metric skips.
#     So v25's black pupil void and brown stroma/limbus were being emitted as invisible backgrounds --
#     collapsing the subject to 4 colors {7,8,11,15} vs the pinned best v7's 6. v7 reached its 6th color
#     with RED (9) at the iris edge -- the recurring speckle defect we are CLOSING, not reopening. So this
#     pass restores the count via NATURAL anatomy instead of red:
#       (a) PUPIL RIM -- the black pupil void rendered as a genuine half-block (fg=0 over a grey sclera
#           bg), i.e. a dark pupil catching a thin rim of lit sclera at its edge. Adds color 0.
#       (b) STROMA->LIMBUS GRADIENT -- the outer iris stroma as brown(3) half-blocks over bright amber(11)
#           bg, i.e. the real collarette-bright -> limbus-dark radial falloff rendered as a half-block ramp
#           instead of a flat fill. Adds color 3.
#     Result palette {0,3,7,8,11,15} = 6 distinct subject colors, coherent amber family, NO red anywhere.
for cr in range(H):
    for px in range(W):
        top=cv.get_pixel(px,cr*2); bot=cv.get_pixel(px,cr*2+1)
        ...
        j=hsh(px,cr,13)
            # (a) PUPIL RIM v29d: light-field-driven FG + BG shading.
            # The flat-region gate uses vis(c)=fg for non-space chars, so we must
            # vary the FG itself, not just the BG. Center=0 (black void), rim=8
            # (dark grey catching sclera glow). This gives 2 distinct visible colors
            # in the pupil alone, breaking any >40-cell contiguous flat region.
        if d < pupil_r+0.4:
            l_pupil = L(px, cr*2+0.5)
            j2 = hsh(px, cr, 31)
             # radial distance from pupil center drives FG: center dark, rim lit
            t_pup = d / max(0.5, pupil_r)   # 0 at center -> 1 at edge
             # FG: center=0 (black), mid=8 (dark grey), rim catches sclera glow
            if t_pup < 0.5:
                pfg = 0                          # deep black void
            elif l_pupil > 0.55:
                pfg = 7                          # lit rim catches sclera glow (light grey)
            else:
                pfg = 8                          # shadow-side rim (dark grey)
             # BG: light field drives the "floor" color
         ...
         # (b) STROMA->LIMBUS: outer amber band as brown-over-amber half-blocks -- the real radial falloff.
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v29.ans","w").write(out)
print("orb v28 = v25 + anatomical foregrounds (black pupil rim, brown stroma->limbus; 6 colors, no red)", len(frame),"rows")
