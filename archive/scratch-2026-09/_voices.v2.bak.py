...
def profile_head(cx, cy, R, dir, ramp):
    ...
    for py in range(ph):
     ...
     # --- neck: a lit column below the head, shaded by its own normal ---------
    for py in range(int(cy + 0.78 * R), int(cy + 1.40 * R)):
        hw = 0.32 * R
        for px in range(int(cx - hw), int(cx + hw) + 1):
            ldx, ldy = Ldir(px, py)
            l = 0.16 + 0.55 * max(0.0, -ldy)         # neck faces up-ish; lit on top
            if (px - cx) * dir > 0:
                l = min(1.0, l + 0.12)
            cv.set_pixel(px, py, ramp(l))
