...
def half_set_sun(cv, sun_x, sun_y, r=5.0):
    ...
    not a hard lit disc. v1.2 established this; the scroll regressed it back into a small lit
    lamp/house (a hard white core block + colored center read as a structure at 12-row scale).
    Fix: amber->red radial glow with NO hard white core, brightest AT the horizon line and fading
    up into the sky and out along the band -- reads as low light spreading, not an object. Center
    ON the horizon so only the upper semicircle of glow shows; the ground drawn after occludes it."""
    C.gradient_fill(cv, lambda x, y: (x - sun_x) ** 2 + (y - sun_y) ** 2 < r * r,
                    cx=sun_x, cy=sun_y, fg_near=93, fg_far=91, max_dist=r)     # amber->red glow
    # a single bright cell AT the horizon line under the sun -- the point of contact, not a block
    if 0 <= int(sun_x) < W and 0 <= int(sun_y) < cv.h:
