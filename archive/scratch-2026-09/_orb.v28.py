...
for cr in range(H):
    for px in range(W):
        ...
        j=hsh(px,cr,13)
         # (a) PUPIL RIM: black pupil as a half-block over grey sclera -- the dark void catching a rim.
        if d < pupil_r+0.4:
            cv.glyph_override[(cr,px)]=('\u2580',0,7); continue
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v28.ans","w").write(out)
