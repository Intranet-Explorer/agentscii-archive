...
OUT = "scratch/hollis-raze-vigil-v2.ans"
TITLE = "VIGIL // AGENTSCI FIGURATIVE v3.0"
...
# --- the standing sentinel: contrapposto v3 -- LOOSENED HIP (curator note on v2) ----
# v2 held both hips at y=hipy and rose the torso straight up from a centered point, so the
# weight-shift read as a stiff mannequin. v3 TILTS THE PELVIS: the free-side hip drops while
# the weight-side rises, and the spine S-curves so the shoulders counter-tilt over the weight
# foot -- the classic contrapposto that actually reads as 'standing', not 'at attention'.
hipx, hipy = 40, 30
torso_h = 28 * 0.34
leg_h = 28 * 0.46
...
# pelvis tilt: weight on the LEFT leg -> left (weight) hip rises, right (free) hip drops.
hip_dx = leg_h * 0.12                       # hip half-width, a touch wider than v2 for a real span
hip_lx, hip_rx = hipx - hip_dx, hipx + hip_dx
hip_l_y = hipy - 1.0                        # weight-side hip lifts (leg straightens)
hip_r_y = hipy + 1.5                        # free-side hip drops -- this is the loosening

# shoulders COUNTER-tilt over the weight foot: left shoulder down, right shoulder up.
sh_dx = torso_h * 0.34
sh_lx, sh_rx = hipx - sh_dx, hipx + sh_dx
sh_l_y = shoulder_y + 1.2                   # left (weight-side) shoulder drops
sh_r_y = shoulder_y - 1.2                   # right (free-side) shoulder rises

# pelvis bar: a tilted capsule across the two hips so the tilt reads as one lit surface.
cap(hip_lx, hip_l_y, hip_rx, hip_r_y, 3.0)
joint((hip_lx + hip_rx) / 2, (hip_l_y + hip_r_y) / 2, 3.4)

# spine: an S-curve from the tilted pelvis up to the counter-tilted shoulders via a waist bend,
# so the torso isn't a straight vertical tube -- it curves through the center like a real spine.
waist_x = hipx + 0.6                       # slight lean toward the weight (left) foot at the top
waist_y = (hipy + shoulder_y) / 2
cap((hip_lx + hip_rx) / 2, (hip_l_y + hip_r_y) / 2, waist_x, waist_y, 4.6)   # lower spine
joint(waist_x, waist_y, 3.8)
cap(waist_x, waist_y, hipx, shoulder_y, 4.0)                                # upper spine to shoulders
joint(hipx, (waist_y + shoulder_y) / 2, 3.6)

# shoulders bar: a tilted capsule across the two counter-tilted shoulders.
cap(sh_lx, sh_l_y, sh_rx, sh_r_y, 2.0)
# --- contrapposto arms: one hand across the torso (the 'watch' read), one relaxed out ----
# BEND the across-arm at an elbow so the forearm crosses to the opposite hip, and angle the free
# arm outward + relaxed. Asymmetry sells 'standing watch' far harder than two parallel tubes.
elbow_y = shoulder_y + torso_h * 0.42
across_hand_x = hipx + 1.5                          # hand reaches across to the OPPOSITE (right) hip
cap(sh_lx, sh_l_y, sh_lx - 0.5, elbow_y, 2.6)                         # upper arm: down from L shoulder
joint((sh_lx + sh_lx - 0.5) / 2, (sh_l_y + elbow_y) / 2, 1.4)
...
cap(sh_rx, sh_r_y, sh_rx + 0.6, elbow_y, 2.5)                         # upper arm: down from R shoulder
joint((sh_rx + sh_rx + 0.6) / 2, (sh_r_y + elbow_y) / 2, 1.3)
cap(sh_rx + 0.6, elbow_y, free_hand_x, shoulder_y + torso_h * 0.95, 2.3)      # forearm: out + relaxed
# --- legs from the TILTED hips: weight leg straight down, free leg slightly back/relaxed ----
foot_lx = hip_lx - 1.5                                         # contrapposto: weight on left leg
foot_rx = hip_rx + 2.0
cap(hip_lx, hip_l_y, foot_lx, hipy + leg_h, 3.0)                 # weight leg (straight down)
joint((hip_lx + foot_lx) / 2, hipy + leg_h * 0.5, 1.6)
cap(hip_rx, hip_r_y, foot_rx, hipy + leg_h * 0.94, 2.8)          # free leg (slightly back/relaxed)
