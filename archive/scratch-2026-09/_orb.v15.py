...
def L(px,py):
    ...
    base = 1.0 - d/80.0
     # gentle continuous variation across the surface so no large uniform plateau forms:
     # a soft radial ripple + angular term -- reads as a lit sphere's curvature, not scatter.
    ang = math.atan2(py-cy, px-cx)
    base += 0.05*math.sin(d*0.18) + 0.04*math.cos(ang*3.0)
...
def aramp(l):
    if l>0.64: return 11                  # bright yellow (lit collarette / lit fiber)
    if l>0.42: return 9                   # orange mid-stroma band -- connected, not a fleck
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v15.ans","w").write(out)
