...
from canvas import sgr, write_ans, shade_ramp
...
# ============================================================
# PASS 6 (v22) -- shade EVERY interior subject cell through a hue-family
# shade_ramp(hot, cold). This is the exact mechanism that cleared beast v5 and
# the accepted guardian v14: density glyphs ▓▒░ are EXCLUDED from the flat-region
# gate's flood-fill (they're evidence of real shading), so converting the whole
# interior to ramped density leaves NO large same-(char,visible-color) patch.
#
# WHY v21 FAILED THE GATE: its dither_flat() used fg=bg (dithers a color into
# itself) AND skipped every cell where top!=bot -- so the two flagged regions
# (blue cranium rows 11-22, grey cloak rows 20-31) were native half-block '▀'
# cells that never got overridden and stayed grouped by visible color. Here we
# override ALL interior cells (fg != bg -> a genuine gradient across the surface),
# keep native ▀ ONLY at silhouette-edge cells so curves stay round, and EXCLUDE
# eye/detail cells so the constructed eyes/ember/brow aren't clobbered (the exact
# bug caught live in beast v5).
# ============================================================
FACE_RAMP = shade_ramp(7, 4, 5)     # light grey -> cold blue: lit cranium/face modelled surface
CLOAK_RAMP = shade_ramp(8, 0, 5)    # dark grey -> black: draped cloth in shade

# Geometric edge test: a cell is a true silhouette edge if ANY of its four
# corner pixels falls outside the silhouette -> keep native ▀ there so the
# cranium/cloak curves stay genuinely round. Interior cells (all 4 corners in)
# get density glyphs, REGARDLESS of top/bot -- the original pass's per-pixel-row
# offset makes nearly every interior cell top!=bot, which would otherwise leave
# them as native ▀ and let the flat-region gate group them by visible color.
def is_sil_edge(col, py_top):
    for cx in (col - 1, col):
        for cy in (py_top, py_top + 1):
            if not in_silhouette(cx, cy):
                return True
    return False

for cell_row in range(cv.h_cells):
    py_top = cell_row * 2
    py_bot = py_top + 1
    for col in range(W):
        if is_sil_edge(col, py_top):
            continue                       # true silhouette edge -> keep native ▀ (round curves)
        if in_detail(col, py_top) or in_detail(col, py_bot):
            continue                       # eye/ember/brow -> constructed detail on top
        hot, cold = (8, 0) if in_shoulder(col, py_top) else (7, 4)
        t = (light(col, py_top) + light(col, py_bot)) * 0.5
        # clamp to the three mid density glyphs only -- NEVER a solid full block or
        # space, both of which the flat-region gate DOES flood-fill. This is exactly
        # the mechanism that cleared the accepted v14: density dither in {▓ ▒ ░}.
        DENS = ['\u2593', '\u2592', '\u2591']      # ▓(75%) ▒(50%) ░(25%)
        b = max(0.0, min(1.0, t + 0.04 * (bayer(col, py_top) - 0.5)))
        lo = int(b * 3.0); hi = min(2, lo + 1)
        frac = b * 3.0 - lo
        thr = BAYER[py_top % 4][col % 4] / 16.0
        idx = hi if thr > frac else lo          # 0..2 -> always a density glyph
        cv.glyph_override[(cell_row, col)] = (DENS[idx], hot, cold)
...
write_ans('scratch/_guardian.v22.ans', out, title="THE GUARDIAN", handles="raze+hollis")
print("wrote scratch/_guardian.v22.ans")
