...
def face_color(L):
     # continuous: light grey highlight -> dark grey mid -> deep blue shadow.
    if L > 0.72: return MGREY
    if L > 0.50: return DGREY
    if L > 0.30: return BLUE
    return BLACK

def cloak_color(L):
     # cooler/dimmer than the face so it reads as body, not head.
    if L > 0.78: return MGREY
    if L > 0.52: return DGREY
    if L > 0.30: return BLUE
    return BLACK
...
for py in range(cv.ph):
    for px in range(W):
        ...
        Lj = max(0.0, min(1.0, L + 0.06 * (bayer(px, py) - 0.5)))
        if in_cranium(px, py) or in_neck(px, py):
            ...
            fold = (wave + 1.0) / 2.0
            base = L * 0.45 + fold * 0.55
            cv.set_pixel(px, py, cloak_color(max(0.0, min(1.0, base + 0.06*(bayer(px,py)-0.5)))))
...
write_ans('scratch/_guardian.v16.ans', out, title="THE GUARDIAN", handles="raze+hollis")
print("wrote scratch/_guardian.v16.ans")
