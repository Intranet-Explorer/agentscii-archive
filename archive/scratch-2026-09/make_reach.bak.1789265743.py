...
FINGERS = [     # (center dx from hand center, length, width) -- pinky..index
    (-6, 10, 3),
    (-2, 14, 3),
    (+2, 15, 3),
    (+6, 12, 3),
]
THUMB_DX, THUMB_LEN = +9, 9            # thumb off the right side of the palm

def draw_hand(p, cx, tip_y, base_y, body=WHITE, glow=CY, through=False):
    """Draw a reaching hand with real silhouette weight. `through` adds a bright
    glow halo at the fingertips -- the moment of breaking through the wall."""
    palm_top = tip_y + 15             # palm sits just below the tallest fingertip
    # -- fingers: adjacent tapered bars fanned in an arc, capped rounded at the tip --
    for dx, flen, fw in FINGERS:
        x0 = cx + dx - fw // 2
        top = tip_y + (15 - flen)      # shorter fingers start lower -> fanned arc of tips
        for y in range(top, palm_top + 4):
            for w in range(fw):
                p.set(x0 + w, y, '\u2588', body, BLACK)
        # rounded tip cap (one row above the bar top, slightly wider)
    ...
    for i in range(THUMB_LEN):
        ...
        if i % 2 == 0:
            p.set(x + 1, y, '\u2588', body, BLACK)     # fatter thumb
    # -- palm: a filled rounded block (the mass that makes it read as a hand) --
    pw = 18
    for dy in range(0, 13):
        y = palm_top + dy
        half = pw // 2 - abs(dy - 6) // 2       # taper top and bottom into knuckles/wrist
        if half < 4:
            half = 4
        for dx in range(-half, half + 1):
            p.set(cx + dx, y, '\u2588', body, BLACK)
    # -- wrist narrowing down to base --
    for dy in range(0, max(0, base_y - (palm_top + 13))):
        y = palm_top + 13 + dy
        half = max(2, 4 - dy // 5)
        for dx in range(-half, half + 1):
            p.set(cx + dx, y, '\u2588', body, BLACK)
        ...
        for dx, flen, fw in FINGERS:
            ...
            for dy in range(-4, 0):
                ...
                if abs(dy) > 1:
                    ...
                    p.set(cx + dx + 1, top + dy, '\u2593', glow, BLACK)
            p.set(cx + dx,     top - 5, '\u2588', WHITE, BLACK)    # white-hot spark
        # a faint bleed halo just above the whole fingertip line
        for dx in range(-7, 8):
            p.set(cx + dx, tip_y - 6, '\u2591', glow, BLACK)
...
def make_reach():
    ...
    p.put_text(3, CX - len("IT RISES") // 2, "IT RISES", MG)
...
def make_break():
    ...
    p.put_text(3, CX - len("BREAKTHROUGH") // 2, "BREAKTHROUGH", YEL)
