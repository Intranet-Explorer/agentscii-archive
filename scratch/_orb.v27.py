...
# --- PASS 10: FOREGROUND HIGHLIGHTS (v27, raze+hollis). The half-block packer collapses any cell
#     whose top==bottom pixel to a plain space+bg, so v25's white sclera crown and amber stroma were
#     being emitted as BACKGROUND -- invisible to the distinct_colors_in_subject metric, which for a
#     non-space glyph counts only the fg (top) pixel. That is why v25 read 4 subject colors vs v7's
#     6-7: its bright highlights never became foregrounds. This pass forces the natural lit highlights
#     to be genuine half-block FOREGROUNDS (bright top pixel, darker bottom), restoring the distinct
#     color count WITHOUT reintroducing red -- the amber stroma stays coherent yellow->brown, no 9/1.
#     Targets: white sclera crown + glint (15), bright-yellow iris collarette near the pupil (11).
for cr in range(H):
    for px in range(W):
        top=cv.get_pixel(px,cr*2); bot=cv.get_pixel(px,cr*2+1)
        if top!=bot: continue                       # already a real half-block curve -- leave it
        l=L(px,cr*2+0.5)
        dx=px-cx; dy=(cr*2+0.5)-cy; d=math.hypot(dx,dy)
        j=hsh(px,cr,11)
        # (a) WHITE sclera crown -- the lit upper-left of the eyeball. Force white as fg over a grey
        #     bottom so it's a genuine half-block highlight, not a flat bg wash. Upper-lit region only.
        if in_opening(px,cr*2+0.5) and l>0.86 and dy<1.5:
            cv.glyph_override[(cr,px)]=('\u2580',15,7); continue
        # (b) GLINT -- the single lit catchlight in the pupil void. Force white fg over black bg so
        #     it reads as a real half-block glint instead of collapsing to a space+bg=0 (invisible).
        if d<3.2:
            cv.glyph_override[(cr,px)]=('\u2580',15,0); continue
        # (c) BRIGHT-YELLOW iris collarette -- the hot ring just outside the pupil. Force 11 as fg over
        #     a brown bottom so the amber reads as a lit gradient (collarette bright -> limbus dark),
        #     not a flat bg fill. This is the coherent amber family, NOT red.
        if in_opening(px,cr*2+0.5) and pupil_r+0.3 <= d < pupil_r+2.6:
            cv.glyph_override[(cr,px)]=('\u2580',11,3); continue
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v27.ans","w").write(out)
print("orb v27 = v25 + foreground highlights (white crown/glint, yellow collarette as fg; no red)", len(frame),"rows")
