    ...
    for r in range(H):
        ...
        for c in range(W):
            ...
            ripple = math.sin(x * 0.7) * math.cos(y * 0.9) * 0.18
             # gentle central brightness peak -> focal hierarchy so the eye has a home,
             # without flattening the field into uniform texture (highball refs all
             # have a clear focal point; pure uniform moire reads as 'noise').
            cxp, cyp = 40.0, 23.0
            peak = math.exp(-((x - cxp) ** 2 + (y - cyp) ** 2) / (2 * 16.0 ** 2)) * 0.35
            v = s + ripple + peak               # ~-1.2..1.5
            ...
            if in_mark:
                ...
                # calm pocket behind the mark: strongly dampen the wave so it's a
                # near-flat DARK trough, with a slow-cycling MUTED (dim) hue. The
                # white-hot letters bloom against this stillness and actually READ,
                # but the pocket still breathes faintly with the wave -> woven, not a
                # painted bar. (bright fg 90-97 must map to dim 30-37, NOT -10.)
                 # calm pocket behind the mark: a NEAR-FLAT DARK ground so the
                 # white-hot letters read as crisp solid blocks. Density is held at
                 # the sparsest ramp end with only a faint wave-driven flicker to
                 # space -> still woven (the ground breathes), not a painted bar.
                 # Hue cycles dimly through muted colors so it's alive, not dead black.
                vv = s * 0.10
                flick = math.sin(x * 0.5 + y * 0.7 + r * 0.3) > 0.86   # rare -> mostly ░
                dens = " " if flick else RAMP[3]      # sparsest end, faint texture
                MUTED = [30, 34, 35, 36]           # dim blue/cyan/magenta/green
                fg = MUTED[int(x * 0.18 + y * 0.10) % len(MUTED)]
            else:
                t = v * 0.5 + 0.5              # 0..1
                if t < 0.0: t = 0.0
                if t > 1.0: t = 1.0
...
emit(sgr(96, 40) + "INTERFERENCE v1.1 -- three-source moire field, wordmark woven in")
emit(RESET)
