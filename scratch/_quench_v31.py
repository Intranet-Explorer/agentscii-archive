...
# ===========================================================================
# PASS 5 -- THERMAL-SHOCK RING (hollis's second-author pass, v3.1 joint).
#   The pressure front of the quench impact: a faint expanding annulus at the OUTER
#   reach of the fins -- where the molten metal has already met cold and is dissipating
#   into the void. Distinct from raze's radial FINS (linear cracks) and point EMBERS
#   (scatter): this is an ANNULAR ring, the one new behavior v3.1 adds to the concept.
#   Warm-only, sparse, low-density so it reads as a cooling shockwave edge, not a solid
#   band -- the bloom's dissipating outer boundary. A few embers ride the ring outward.
# ===========================================================================
SHOCK_R = CORE_R + 24.0          # just beyond the longest fin reach -> the impact front
...
for y in range(H2):
    for x in range(W):
        ...
        # break it up so it reads as a dissipating shockwave, not a clean circle: sparse + noisy
        n = noise(x, y)
        if n < 0.55:
            continue
        h *= (0.45 + 0.55 * n)
        # slow angular shimmer so the ring isn't perfectly uniform
        ang = math.atan2(y - CY, x - CX)
        h *= (0.7 + 0.3 * math.sin(ang * 6.0 + d * 0.4))
        if h < 0.18:
            continue
        fg = warm_fg(h, d * 0.25 + ang * 2.0)
        set_cell(cv, x, y, heat_to_density(min(1.0, h)), fg)

# a few embers riding the shock ring outward (the last sparks leaving the impact front)
for _ in range(26):
    ang = random.uniform(0, 2 * math.pi)
    ...
    if h < 0.3 or random.random() < 0.5:
        continue
    fg = 15 if h > 0.8 else warm_fg(h, r * 0.4)
...
sig = []
sig_block(sig, "QUENCH // CONFLAGRATION v3.1", handles="raze,hollis")
