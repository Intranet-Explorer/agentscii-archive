...
def _grain(x, y, amp=0.16):
    """Stable per-cell micro-variation in [-amp, amp] -- the v10 fix for HORIZONTAL BANDING.
    N.L with an x-dominant light gives every cell at similar x a similar density (one band per row);
    a deterministic per-cell jitter makes density vary DOWN a row too. Same seed -> coherent, not noise."""
    h = ((x * 73856093) ^ (y * 19349663)) & 0xFFFFFFFF
    return (((h >> 16) / 65535.0) - 0.5) * 2.0 * amp
...
def nl(cx, cy, rx, ry, x, y, amb=0.10):
    ...
    L += _grain(x, y)                 # per-cell variation -> kills the horizontal banding
    L += ((y - cy) / ry) * 0.12        # gentle vertical gradient -> density varies DOWN a row too
...
paint(lambda x, y: in_mask(x, y) and cranium_hw(y) is None,
      MID, 32.0, 15.0, 9.0, base_fg=7, hot_fg=15, amb=0.08, white_at=0.99)
...
def eye_socket(x, y):
    ...
    return min(dL, dR) <= 3.0

# v10: the eyes are RECESSED WELLS, not flat amber squares -- a dim grey socket ring carved into the
# cheekbone with a small amber glint set inside it. The "watches back" read now has structure to look at.
for y in range(H):
    for x in range(W):
        if eye_socket(x, y):
            cv.set(x, y, '\u2591', 8, 0)                 # shadowed socket wall (dim grey, recessed)
# amber glint: a small lit core inside each well -- the single accent color of the whole piece
for sx in (MID - 8, MID + 8):
    for dy in (-1, 0, 1):
        cv.set(int(sx), 21 + dy, '\u2588', ACCENT, 0)
# a single white catch-light at each eye's upper-left (light side) -- the "watches back" glint
for sx in (MID - 8, MID + 8):
...
out = []
cv.render(out)
sig_block(out, "TOTEM v10.0 (N.L stone, grain+sockets)", handles="raze")
raw = "\n".join(out) + "\x1b[0m\n"
