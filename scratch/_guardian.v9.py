...
for py in range(cv.ph):
    for px in range(W):
        ...
        # coherent vertical folds: continuous in x with gentle vertical drift + a little
        # per-column phase, so bands read as cloth drape, not per-column random stripes.
        fold = math.sin(px * 0.50 + py * 0.10 + _phase[px] * 0.35)   # -1..1
        amp = 0.18 + t * 0.22                                       # visible, grows to hem
        Lf = L + fold * amp
...
write_ans('scratch/_guardian.v9.ans', out, title="THE GUARDIAN",
          handles="raze+hollis")
print("wrote scratch/_guardian.v9.ans")
