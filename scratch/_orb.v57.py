...
SCLERA_RAMP = ['\u2580','\u2593','\u2592','\u2591']   # full -> sparse (density)
for cr in range(H):
    for px in range(W):
        u=(px-cx)/half_w
        ...
        # density index from the curved light: bright pole -> full block, rim -> sparse.
        didx = int((1.0 - lcur) * 3.99) % 4
        jz = hsh(px, cr, 77)
        if jz > 0.85: didx = (didx+1)%4
        elif jz < 0.12: didx = (didx-1)%4
        glyph = SCLERA_RAMP[didx]
        # color follows light: bright pole white, mid gray, rim dark-gray on black.
        if lcur > 0.62: fg,bg = 15,7
        elif lcur > 0.38: fg,bg = 7,8
        else: fg,bg = 8,0
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v57.ans","w").write(out)
print(f"orb v57 = v56 + curved lit sclera surface fix (Opus claim #2: flat/banded sclera -> density-ramp curvature, same upper-left light source as iris), {len(frame)} rows")
