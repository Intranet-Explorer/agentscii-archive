...
def hue_ramp(Lv):
    ...
    if   Lv > 0.78: fg = 93            # white-hot lit crest (the sunlit right edge)
    elif Lv > 0.56: fg = 92            # bright yellow -- the lit mass
    elif Lv > 0.34: fg = 107           # warm magenta mid
    elif Lv > 0.18: fg = 91            # dim red shadow (the far side receding)
...
for x in range(W):
    ...
    if Lr > 0.06:
        fg = 15 if Lr > 0.7 else (92 if Lr > 0.45 else (8 if Lr > 0.2 else 8))
