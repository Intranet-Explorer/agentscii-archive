...
DT = 0.40                                 # advection step size (cells) -- sub-stepped smoothness
DRIFT = 0.55                              # prevailing "wind" term added to every local angle
...
for p in range(PARTICLES):
    ...
    for s in range(LIFE):
        ...
        x += math.cos(ang) * DT
        y += math.sin(ang) * DT
        if not (0 <= x < W or 0 <= y < H):
            x = x % W; y = y % H          # v3: wrap -- keep the path continuous, no respawn break
...
sig2 = "FLOWFIELD v3.0 -- curl-noise ribbons"
def sigline(text, fg):
    pad = W - len(text)
    left = pad // 2
    right = pad - left
...
text = "\n".join(out)
with open("scratch/hollis-raze-flowfield-v3.ans", "w", encoding="cp437") as f:
    f.write(text)

# --- self-check hygiene gate --------------------------------------------------
raw = open("scratch/hollis-raze-flowfield-v3.ans", "rb").read()
