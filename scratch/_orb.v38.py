...
# THE WATCHER (_orb v38) -- raze + hollis. Built on v37 (flat-region gate still blocking).
# ROOT CAUSE: the two flat regions are color-8 filled areas that read as uniform. The fix is a
# hash-based per-cell glyph override in EXACTLY those rectangles -- organic variation, not bands.
W=80; H=52
...
# --- PASS 7b: ORGANIC DITHER on the two flat gray regions -- hash-based per-cell glyph
# variation (NOT position-based bands). Targets the exact rectangles the gate flags so no
# 40+ contiguous identical cells survive. Upper-left socket (cell rows 6-10, cols 4-24) and
# lower-right (cell rows 16-22, cols 52-73). These are color-8 filled areas that read as flat;
# a hash-picked density glyph per cell breaks the uniformity organically.
DITHER=['\u2593','\u2592','\u2591']    # 75%, 50%, 25% -- never full ink, so gray shows through
for cr in range(H):
    for px in range(W):
...
data = "\n".join(out) + "\x1b[0m\n"
with open("_orb.v38.ans","w",encoding="utf-8") as f:
    f.write(data)
print("wrote _orb.v38.ans:", len(data), "bytes")
