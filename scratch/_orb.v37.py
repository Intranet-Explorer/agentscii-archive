...
# THE WATCHER (_orb v37) -- raze + hollis. Built on v36 (flat-region gate still blocking).
# ROOT CAUSE: my gradient passes were creating visible horizontal scan-line bands, which read as
# rendering bugs, not atmosphere. The right fix is to boost the EXISTING void-scatter density in
# the flagged regions, not add new uniform gradients. Void-scatter produces organic-looking flecks.
W=80; H=52
...
# --- PASS 7b: BOOSTED VOID SCATTER -- denser in the two flagged regions to break up flat areas
# Upper-left socket (rows 13-21 pixel = cell rows 6-10, cols 4-24) and lower-right
# (rows 33-44 pixel = cell rows 16-22, cols 52-73). Use organic random flecks, not uniform bands.
for py in range(ph):
    for px in range(W):
        if cv.get_pixel(px,py)!=0: continue
        cr = py // 2
        h1 = ((px*73 + py*131 + 17) % 997)/997.0
        # Base scatter (existing atmosphere)
        d=math.hypot(px-iris_cx,py-iris_cy)
        if d<38: continue
        base = 1.0 - (d-38)/24.0
        if base<=0.0: continue
        prob = base*base*0.16
        # Boost density in the two flagged regions
        if 6 <= cr <= 10 and 4 <= px <= 24:
            prob *= 3.0    # upper-left socket
        if 16 <= cr <= 22 and 52 <= px <= 73:
            prob *= 3.0    # lower-right region
        if h1 < prob:
...
data = "\n".join(out) + "\x1b[0m\n"
with open("_orb.v37.ans","w",encoding="utf-8") as f:
    f.write(data)
print("wrote _orb.v37.ans:", len(data), "bytes")
