...
def pass4():
    # FLAT interior: a uniform near-black body. NO grain in the fill and NO radial
    # density ramp -> kills the horizontal banding that fought the silhouette (hollis
    # v2 critique point a). The moon reads as ONE clean dark object by CONTRAST against
    # the corona, not as a banded disc.
    for y in range(MOON_CY-MOON_R-1, MOON_CY+MOON_R+2):
        for x in range(MOON_CX-MOON_R-1, MOON_CX+MOON_R+2):
            ...
            d = math.hypot(dx, dy)
            if d <= 1.0:
                place(x, y, "\u2588", 0)               # solid near-black body, uniform
    # faint earthshine on the LIT outer limb only -- a thin rim, not a wash. Keeps it
    # reading as a modeled body rather than a flat hole, with zero interior banding.
    for y in range(MOON_CY-MOON_R-1, MOON_CY+MOON_R+2):
        for x in range(MOON_CX-MOON_R-1, MOON_CX+MOON_R+2):
            dx = (x-MOON_CX)/MOON_R; dy = (y-MOON_CY)/MOON_R
            d = math.hypot(dx, dy)
            if 0.94 <= d <= 1.0:
                ...
                if dot > 0.28:
                    place(x, y, "\u2593", 8)           # faint warm-grey rim on lit limb

# ---- PASS 5: the DIAMOND-RING -- a genuinely THIN white-hot arc on the LIT limb,
# ISOLATED from the corona band by a thin dark moat so it reads as a crisp line of
# light (the last sliver of sun at the limb), not smeared into the warm glow. This is
# hollis v2 critique point b: the ring was smearing into the corona; now it pops.
def pass5():
    ring_r = 1.035                                 # just outside the moon edge, ~1 cell thick
    for y in range(MOON_CY-MOON_R-3, MOON_CY+MOON_R+4):
        for x in range(MOON_CX-MOON_R-3, MOON_CX+MOON_R+4):
            dx = (x-MOON_CX)/MOON_R; dy = (y-MOON_CY)/MOON_R
            d = math.hypot(dx, dy)
            nx, ny = dx, dy
            nz = math.sqrt(max(0.0, 1.0 - min(dx*dx+dy*dy,1.0)))
            dot = nx*LX + ny*LY + nz*0.40          # lit crescent only
            if dot <= 0.30: continue               # ring lives on the LIT side only
            dd = abs(d - ring_r)
            if dd < 0.16:                         # carve a thin dark moat around the arc
                place(x, y, "\u2591", 0)         # near-black gap isolates the white line
    for y in range(MOON_CY-MOON_R-3, MOON_CY+MOON_R+4):
        for x in range(MOON_CX-MOON_R-3, MOON_CX+MOON_R+4):
            dx = (x-MOON_CX)/MOON_R; dy = (y-MOON_CY)/MOON_R
            d = math.hypot(dx, dy)
            ...
            if abs(d - ring_r) < 0.05:            # ~1-cell-thick white arc
                place(x, y, "\u2588", 15)
