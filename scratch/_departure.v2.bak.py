...
LX, LY = 26.0, 18.0
def L(x, y):
    return fc.light_field(x, y, LX, LY, lmax=34.0, ambient=0.18)
...
def lit_wheel(Li):
    ...
    if Li < 0.30: return STEEL
    if Li < 0.50: return GREY
    if Li < 0.72: return LAMP
    if Li < 0.92: return LAMP_HI
    return 15
...
def make_keeper():
    ...
    joint_tube(p, LAMPX, LAMPY, 2.2)          # the lamp head (warm, lit)
    for y in range(LAMPY - 3, LAMPY + 3):
        for x in range(LAMPX - 3, LAMPX + 4):
            if math.hypot(x - LAMPX, y - LAMPY) <= 2.6:
                setc(p, x, y, '█', LAMP_HI, 0)
    lamp_glow(p, LAMPX, LAMPY, R=13)          # the warm glow pool that grounds everything
    ...
    capsule_tube(p, HIPX, HIPY, CHESTX, CHESTY, halfw=3.6)
    joint_tube(p, HIPX, HIPY, 3.0)           # pelvis
    joint_tube(p, CHESTX, CHESTY, 4.4)       # chest/shoulders
    ...
    fc.brow_ridge(p.canvas, HEADX - 1.5, HEADCY - 0.5, halfw=2.4, light=L, base_fg=GREY_HI, hot_fg=LAMP_HI)
    ...
    capsule_tube(p, CHESTX - 1.5, CHESTY + 1, 38.0, 24.0, halfw=2.2)
    joint_tube(p, 38.0, 24.0, 2.4)
    capsule_tube(p, 38.0, 24.0, 33.0, 28.0, halfw=1.6)   # forearm down to a held lantern
    # RIGHT arm (away from light -- in shadow, cool steel)
    capsule_tube(p, CHESTX + 1.5, CHESTY + 1, 53.0, 27.0, halfw=2.2)
    joint_tube(p, 53.0, 27.0, 2.4)
    capsule_tube(p, 53.0, 27.0, 54.0, 33.0, halfw=1.6)

    # legs -- a standing contrapposto stance (weight on one leg), lit tubes
    capsule_tube(p, HIPX - 1.2, HIPY + 1, 44.0, 45.0, halfw=2.3)   # weight-bearing left thigh
    joint_tube(p, 44.0, 45.0, 2.1)
    capsule_tube(p, 44.0, 45.0, 43.0, 50.0, halfw=1.6)             # left shin to foot
    capsule_tube(p, HIPX + 1.2, HIPY + 1, 49.0, 45.0, halfw=2.3)   # relaxed right thigh
    joint_tube(p, 49.0, 45.0, 2.1)
    capsule_tube(p, 49.0, 45.0, 51.0, 50.0, halfw=1.6)             # right shin to foot

    # the held lantern (warm glow in the left hand) -- a second small light accent
    lamp_glow(p, 32.0, 29.0, R=5)
    joint_tube(p, 32.0, 28.0, 1.4)
