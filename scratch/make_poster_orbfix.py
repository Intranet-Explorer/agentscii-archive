...
for y in range(H_INNER):
    for x in range(INNER_W):
        ...
        if (x*3 + y*7) % 4 == 0:   # ORBFIX: dense cycling wash field
...
# ---------------------------------------------------------------------------
CCX, CCY = CX, 20
RX, RY = 15.0, 9.0                         # ORBFIX: bigger so the orb actually dominates
for y in range(H_INNER):
    for x in range(INNER_W):
        dx=(x-CCX)/RX; dy=(y-CCY)/RY
        r=math.sqrt(dx*dx+dy*dy)
        if r>1.0: continue
        ring=int(r*3.0)                           # ORBFIX: more rings -> reads as a sphere, not flat disc
        col=hue(ring + y*0.10)                    # slow phase shift -> bands stay colored, not grey
        ch='█' if r<0.45 else ('▓' if (x+y)%2==0 else '▒')    # ORBFIX: bigger white-hot core
        set_cell(x, y, ch, 15 if r<0.45 else col, 0)     # white-hot core -> clear focal point

# ORBFIX: dropped the destructive black clear-ring (it fragmented the orb into scattered blocks).
# A thin bright rim isolates the hero against the dense wash instead of clearing a hole in it.
for y in range(H_INNER):
    for x in range(INNER_W):
        dx=(x-CCX)/RX; dy=(y-CCY)/RY
        r=math.sqrt(dx*dx+dy*dy)
        if 0.95 < r <= 1.02:
            set_cell(x, y, '█', 14, 0)           # thin bright cyan rim -> crisp circle edge
...
open("scratch/hollis-raze-poster.orbfix.ans", "w").write("\n".join(lines))
print("wrote scratch/hollis-raze-poster.orbfix.ans     (%d inner rows, %d total lines)" % (H_INNER, len(lines)))
