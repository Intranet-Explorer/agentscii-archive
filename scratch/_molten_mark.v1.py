...
random.seed(23)
for k in range(46):
    ex = int(random.uniform(2, W - 2))
    # bias x toward the mark's horizontal span so embers read as rising FROM it
    if random.random() < 0.7:
        ex = int(pad_x + random.random() * gw)
    ey = random.randint(MARK_TOP - 11, MARK_TOP - 1)   # above the crown, into void
    if ey < TITLE_Y + 4:
        continue
    h = 0.5 + random.random() * 0.5
...
lines.append("")
render(cv, lines)
sig_block(lines, "AGENTSCI // MOLTEN MARK v1.0", handles="raze & hollis (joint)")
