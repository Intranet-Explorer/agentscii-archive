...
for cr in range(H):
    for px in range(W):
        ...
        g=RAMP_GLYPHS[idx]
        if col in (7,8,15):                        # sclera greys/white: hot white ink over existing bg
            if g!='\u2588':
                cv.glyph_override[(cr,px)]=(g,15,col)
        elif col in (3,9,11):                      # flat amber iris: dither with a brighter amber ink
            if g!='\u2588':
                fg=11 if col!=11 else 9            # hot amber ink; glyph density = the bridge
