...
# v4 (raze, on hollis's two-move critique of v3): the figure read as a thin bright stripe in
# BOTH panels -- the pillar/monolith problem we fixed in static traveler v1.2. Root cause:
# hooded_figure lit the robe from its OWN center (light_field at fx+lean*2), so hot_fg=15 ran
# straight down the midline; and it sat in the busy per-cell ripple field at the same brightness,
# so the rounded cross-section never resolved into a robe. Fixes:
#   (a) directional light -- source offset toward the SUN side -> bright sun-side edge, dim far
#       side across the robe (a lit 3D surface, not a symmetric stripe);
#   (b) wider hem (~6.5) so the flare resolves at this scale;
#   (c) a dark cast-shadow streak off the sun side at the feet -> separation by CONTRAST;
#   (d) feathered ripple damping in a column behind each figure (Gaussian, no hard box) so the
#       outline reads against calmer space without a rectangular patch cut into the field.
...
def field_row(cv, y, t01, sun_x, sun_y, glow_r, hue_phase, ripple_amp=0.06,
              calm_cx=None, calm_hw=5.0):
    """One row of the textured horizon field: vertical value ramp (t01) + warm halo near the
    sun + a gentle per-cell ripple for receding-space texture (not flat bands). When calm_cx is
    set, the RIPPLE is feathered down to ~0 in a column around calm_cx (Gaussian falloff, NOT a
    hard box) so a figure sitting there reads its outline against calmer space while hue and value
    stay continuous with the field -- no rectangular patch cut into it."""
    ...
    for x in range(W):
        ...
        amp = ripple_amp
        if calm_cx is not None:
            dd = abs(x - calm_cx) / max(1e-6, calm_hw)
            amp *= _m.exp(-(dd * dd))                          # feathered damping, no hard edge
        ripple = amp * _m.sin(x * 0.5 + y * 0.7)
...
def half_set_sun(cv, sun_x, sun_y, r=5.0):
    """The recurring mark: a HALF-SET sun -- a soft warm glow that sinks INTO the horizon band,
    not a hard lit disc. The scroll v1 regressed this into a small lit lamp/house two ways: (a) a
    hard white core block, and (b) gradient_fill painting its UPPER semicircle up into the sky --
    nothing occludes it, so it floats as a warm blob with a bright top. Real fix: clip the glow to
    y >= sun_y (at/below the horizon line only). The upper half is gone by construction, so only a
    soft amber->red wash spreads DOWN into the near field and OUT along the band -- low light
    sinking, not an object. A single white cell marks the point of contact on the line."""
    sun_x, sun_y, r = int(round(sun_x)), int(round(sun_y)), int(round(r))
    for y in range(int(sun_y), int(sun_y) + r + 1):             # lower half only -> truly half-set
        if y >= cv.h:
            break
        for x in range(int(sun_x - r), int(sun_x + r) + 1):
            if not (0 <= x < W and 0 <= y < cv.h):
                continue
            d = ((x - sun_x) ** 2 + (y - sun_y) ** 2) ** 0.5
            if d > r:
                continue
            t = d / max(1e-6, r)
            ch = RAMP[min(3, int(t * 4))]                        # amber near center -> red out
            fg = 93 if t < 0.5 else 91                            # warm: yellow core, red rim
            cv.set(x, y, ch, fg, 0)
    if 0 <= int(sun_x) < W and 0 <= int(sun_y) < cv.h:           # the point of contact on the line
        cv.set(int(sun_x), int(sun_y), "\u2588", 15, 0)

def hooded_figure(cv, fx, feet_y, height=14.0, lean=0.0, sun_x=None):
...
for y in range(0, HORIZON_Y):                       # sky: value ramp + warm halo + ripple texture
    ...
    field_row(cv, y, t, SUN_X_I, float(HORIZON_Y), 34.0, hue_phase=2.0, ripple_amp=0.06,
                calm_cx=(FIG_X_I if HORIZON_Y - 13 <= y <= HORIZON_Y + 2 else None))
...
half_set_sun(cv, W / 2.0, float(H2 // 2), r=4.5)                             # the mark, centered
...
for y in range(0, HORIZON_Y):
    ...
    field_row(cv, y, t, SUN_X_II, float(HORIZON_Y), 34.0, hue_phase=2.0 + 4.0, ripple_amp=0.06,
                calm_cx=(FIG_X_II if HORIZON_Y - 13 <= y <= HORIZON_Y + 2 else None))
