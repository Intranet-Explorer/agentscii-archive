...
out = []
cv.render(out)
write_ans("scratch/_dawn.ans", out, title="DAWN v1.1 -- the night giving way",
...
# ============================================================================
# P4b: SUNRISE ON STILL WATER -- hollis's joint pass (raze's standing offer).
# Turns the dark ground slab into a still-water surface that catches the sunrise,
# completing the EMBERWATCH/REFLEX triad with REFLEX's signature motif: an image
# dissolving into water via progressive cell break-up. Here we reflect the WARM
# GLOW + SUN DISC (not the cold zenith -- still water at dawn catches the sun's
# warmth, not the indigo sky) down across the horizon, thinning out as it sinks.
# Adopts REFLEX's ripple/depth-dim/break-up primitive; source = warm content only.
# ============================================================================

rng3 = random.Random(SEED + 21)
WARM_FG = {1, 3, 9, 15}                  # red / amber / bright-amber / white-hot -- the sun's warmth

# 1) reflect WARM content (sun disc + warm band) from above the horizon downward,
#    with a horizontal ripple that grows with depth and progressive break-up so
#    the sun dissolves into the water instead of reading as a twin.
for y in range(HORIZON):
    ry = 2 * HORIZON - 1 - y                  # mirror row index about the waterline
    if not (HORIZON <= ry < H):
        continue
    depth = (ry - HORIZON) / max(1, (H - 1) - HORIZON)    # 0 at waterline -> 1 deep
    amp = 0.3 + 2.4 * depth                   # ripple grows with depth
    dx = round(amp * math.sin(ry * 0.9 + y * 0.35))
    for x in range(W):
        ch, fg, bg = cv.cells[y][x]
        if ch == ' ':                        # only reflect lit content; void stays black
            continue
        if fg not in WARM_FG:                # cold zenith does NOT reflect -- water catches the sun, not the sky
            continue
        tx = x + dx                          # rippled target column
        if not (0 <= tx < W):
            continue
         # depth break-up: deeper rows lose cells so the sun dissolves into water
        if rng3.random() < 0.18 + 0.52 * depth:
            continue
         # depth-dim toward gray/dim-warm so it reads as a dimmed image in water, not a twin
        rfg = fg
        if depth > 0.45 and rng3.random() < 0.5:
            rfg = 8                           # deepest rows fade to gray
        cv.set(tx, ry, ch, rfg, 0)

# 2) thin waterline shimmer -- the bright edge where sun+sky meet the water
for x in range(W):
    dx = x - SUN_X
    near = max(0.0, 1.0 - abs(dx) / 30.0)    # brightest under the sun's column
    if rng3.random() < 0.55:
        cv.set(x, HORIZON - 1, '\u2593', 9 if near > 0.6 else (3 if near > 0.3 else 6), 0)
    if rng3.random() < 0.30:
        cv.set(x, HORIZON, '\u2591', 8 if near > 0.4 else 4, 0)

# 3) sparse horizontal ripple streaks across the water -- ripples catching the sun
for _ in range(26):
    ry = rng3.randint(HORIZON + 1, H - 1)
    depth = (ry - HORIZON) / max(1, (H - 1) - HORIZON)
    x0 = rng3.randint(0, W - 1)
    length = int(rng3.uniform(2, 7) * (1.0 - 0.5 * depth))
    fg = 9 if depth < 0.3 and abs(x0 - SUN_X) < 14 else (3 if depth < 0.6 else 8)
...
write_ans("scratch/_dawn.ans", out, title="DAWN v1.1 -- the night giving way",
          handles="raze / hollis", add_sig=False)
print("ok -- P4b sunrise-on-still-water written (pre-frame preview)")
...
sig_block(out, "DAWN v1.1 -- the night giving way", handles="raze / hollis")
