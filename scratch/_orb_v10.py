...
# ORB v10 -- "THE WATCHER": a single lit EYE in a dark void. HalfBlockCanvas (2px/cell).
...
# --- PASS 7: VIGNETTE / GLOW -- coherent radial glow hugging the eye, fading to pure black.
for py in range(ph):
    for px in range(W):
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v10.ans","w").write(out)
print("orb v10", len(frame),"rows")
