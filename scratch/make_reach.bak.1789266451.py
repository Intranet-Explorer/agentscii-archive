...
def draw_hand(p, cx, tip_y, base_y, body=WHITE, glow=CY, through=False):
    """Draw a compact reaching hand with real silhouette weight. `through` adds a
    bright glow halo at the fingertips -- the moment of breaking through the wall."""
    palm_top = tip_y + 13               # palm sits just below the tallest fingertip
    # -- fingers: thick adjacent bars fanned in an arc, capped rounded at the tip --
    for dx, flen in FINGERS:
        ...
        for w in range(-1, FW):
            p.set(cx + dx - 1 + w, top - 1, '\u2588', body, BLACK)
