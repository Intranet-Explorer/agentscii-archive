...
def main():
    ...
    brow_ridge(cv, HERO_X, HEAD_CY - 1.5, 4.2)
    eye(cv, HERO_X - 3.0, HEAD_CY + 0.5, r=1.6, iris_fg=CYAN)
    eye(cv, HERO_X + 3.0, HEAD_CY + 0.5, r=1.6, iris_fg=CYAN)
    nose(cv, HERO_X, HEAD_CY + 1.5, HEAD_CY + 4.5)
    mouth(cv, HERO_X, HEAD_CY + 6.5, 2.5)

    # ---- P4: warm glow pool at the hero's base -- "the lit one" steps out of the cold crowd ----
    for y in range(int(HEAD_CY + HEAD_RY * 0.5), H - 1):
        for x in range(int(HERO_X - 12), int(HERO_X + 13)):
            if not cv.in_bounds(x, y):
                continue
            d = math.hypot(x - HERO_X, y - (H - 4)) / 13.0
            cur = cv.get(x, y)
            if cur[0] == ' ' and random.Random(SEED + x * 7 + y).random() < (1.0 - d) * 0.5:
                cv.set(x, y, RAMP[2], AMBER if d < 0.4 else ORANGE, 0)
