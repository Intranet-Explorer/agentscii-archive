...
def main():
    ...
    for gx in range(0, W + 1, 4):
        for y in range(22, H - 1, 2):
            ...
            if 0 <= xx < W and 0 <= y < H:
                g = max(4, int(4 + t * 8))    # dimmer far, brighter near -- depth in the floor itself
                set_cell(cv, xx, y, '\u2591', g, 0)
    ...
    for (hip_y, fh, bfg, hfg, count, accent) in ROWS:
        ...
        xs = [3 + i * ((W - 6) / max(1, count - 1)) + rng.uniform(-1.2, 1.2) for i in range(count)]
        for x in xs:
            standing_figure(cv, x, hip_y, L, height=fh, stance="contrapposto",
