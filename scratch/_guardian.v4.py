    ...
    for px in range(W):
        for side in (-1, 1):
            if in_eye(px, py, side):
                ...
                cv.set_pixel(px, py, EMBER_H if d < 1.2 else EMBER)

# faint ember halo just outside the slits -- SAME skew as the slit so the glow
# follows the slit instead of landing on cranium pixels (fixes orphan blob #4)
for py in range(cv.ph):
    for px in range(W):
        for side in (-1, 1):
            ex = CX + side * EYE_DX
            skew = int((py - EYE_Y) * 0.7)
            d = math.hypot(px - (ex + skew), py - EYE_Y)
            if 4.5 <= d <= 6.5 and in_cranium(px, py):
                cur = cv.get_pixel(px, py)
                if cur == BLACK or cur == BLUE:
                    cv.set_pixel(px, py, EMBER)

# cheek / jaw hint: a soft shadow under the eye line on the far (right) side so
# the head reads as a modelled form, not two flat blocks. Light comes upper-left,
# so the lower-right of the face is in deeper shadow.
for py in range(CRAN_CY - 4, NECK_Y + 1):
    for px in range(W):
        if in_cranium(px, py) and px > CX and py > EYE_Y:
            cur = cv.get_pixel(px, py)
            # deepen the shadow on the far cheek/jaw toward the bottom-right
            if cur == DGREY and light(px, py) < 0.5:
                cv.set_pixel(px, py, BLUE)
    ...
    for px in range(W):
        ...
        if abs((px - CX)) >= half - 2 and abs((px - CX)) <= half:
            if px < CX and light(px, py) > 0.35:
                # contiguity: only paint if the pixel just inside is also cape
...
write_ans('scratch/_guardian.ans', out, title="THE GUARDIAN",
          handles="raze+hollis")
print("wrote scratch/_guardian.ans (v4)")
