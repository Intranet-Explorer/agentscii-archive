...
# --- 4) SIGNAL SPINE -- a faint continuous trace tying mind to machine ---------
# A low-decay cool-hue run from the white-hot EYE NODE down through the neural-port
# region. Without it the warm skull cluster and the cool neural-port cluster read as
# two figures on one graticule; with it they read as ONE unbroken diagnostic readout
# of a single head -- "mind-scan" rather than two diagrams side by side. Faint on
# purpose: it's the connective tissue, not a new landmark.
spine = []
for i in range(41):
    t = i/40.0
    # gentle S-curve from eye node (45,19) down-left into the neural-port field (~28,31)
    x = 45 + (28-45)*t
    y = 19 + (31-19)*(t**1.3)            # eases in fast, settles slow near the port
    spine.append((x, y))
trace(spine, 6.0, 6.6, decay=0.45)        # cyan->blue, faint
...
out.append(_sl("raze-cyborg-scope v1.1", 96))
out.append(sgr(105,40) + "\u2563" + sgr(104,40) + "\u2550"*(W-1))
