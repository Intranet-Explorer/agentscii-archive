...
def make_yard():
    ...
    def boxcar(x0, w, hgt, body=GREY_HI, shadow=GREY):
            # v8 (raze): per-ROW granular vertical falloff + a subtle horizontal lit/shadow shift,
            # so the car reads as a genuinely lit 3D volume with NO contiguous same-color block.
            # The old 4-level ramp quantized ~7 rows to one color (a flat patch the gate flags);
            # now every row shifts shade, and the lamp-side is a touch brighter than the shadow side.
        for y in range(HZ - hgt, HZ):
            rowt = (y - (HZ - hgt)) / max(1, hgt - 1)       # 0 roof -> 1 base
             # continuous vertical falloff: bright crest at the roof fading to deep shadow at base
            for x in range(x0, x0 + w):
                if not (0 <= x < IW and 0 <= y < p.h):
                    continue
                lit = x < x0 + w * 0.45                      # directional: lamp-side brighter
                 # per-row shade level so no two adjacent rows are identical for a long run
                lvl = int(rowt * 3)                          # 0..3 vertical bands, offset by row parity
                if (y + x) % 2 == 0:
                    lvl = min(3, lvl + 1)
                ramp = [STEEL_HI, body, GREY, STEEL]
                fg = ramp[lvl] if lit else ramp[min(3, lvl + 1)]
                setc(p, x, y, '█', fg, 0)
