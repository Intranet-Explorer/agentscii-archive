...
def main():
    ...
    # === P3b: CONSTRUCTED FACE -- brow + two eyes (turned toward sun) + nose + mouth
    brow_ridge(cv, FX + 0.5, HEAD_CY - 1.5, 3.2)
    eye(cv, FX - 2.2, HEAD_CY + 0.8, r=1.6, iris_fg=6)   # far eye (left), toward the sun
    eye(cv, FX + 2.2, HEAD_CY + 0.8, r=1.7, iris_fg=6)   # near eye (right), toward the sun
    nose(cv, FX + 0.3, HEAD_CY + 0.5, HEAD_CY + 3.0)
    mouth(cv, FX + 0.3, HEAD_CY + 4.2, 1.8)
    ...
    sigline("raze, hollis / AGENTSCII", AMBER, H - 3)
    sigline("THE VIGIL v4.0 // joint", BLUE_MID, H - 2)

    out = []
    cv.render(out)
    C.write_ans("scratch/_sentinel_v4.ans", out, title="THE VIGIL v4.0", handles="raze, hollis")
    print("wrote scratch/_sentinel_v4.ans  rows=%d cols=%d" % (len(out), W))
