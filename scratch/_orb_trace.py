...
def count_foreign(label):
    ...
    print(f"{label}: non-amber in annulus = {cnt}")
...
for cr in range(H):
    ...
    for px in range(W):
        ...
        up,lo=opening_half_h(u); dy=pyc-cy
        if not(-up<=dy<=lo): continue
        l=L(px,pyc); vert=1.0-(dy/max(1.0,(up+lo)))*0.26; l=max(0.03,min(1.0,l*vert))
        col=gramp(l); cv.set_pixel(px,cr*2,col); cv.set_pixel(px,cr*2+1,col)
count_foreign("after sclera")
...
for cr in range(H):
    ...
    for px in range(W):
        ...
        if 0<=dy<4 and -up<=dy<=lo:
            ...
            elif cur==7 and 1<=dy<3: cv.set_pixel(px,cr*2,8); cv.set_pixel(px,cr*2+1,8)
count_foreign("after shadow band")
...
count_foreign("after glow (full v9)")

# PROPOSED FIX: cleanup pass -- any non-amber pixel INSIDE the annulus gets amber,
# computed from the radial gradient so it's coherent, not a flat patch.
for py in range(ph):
    for px in range(W):
        if not in_opening(px,py): continue
        d=math.hypot(px-cx,py-cy)
