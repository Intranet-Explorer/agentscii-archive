...
GRID = 16                        # v4b: sparse uniform lattice -- faint scope field, not a mesh
...
GLOW_R = 26.0                        # glow reach (cell space) from the light source
# PASS 4: SPARSE radial glow -- falloff carried by DENSITY + DIMMING, not by filling a ring.
#      A filled per-row band reads as horizontal stripes (the v1 'slicing' defect). Instead we paint
#      only where the glow is strong AND on a coarse jittered lattice, dimming with distance, so it
#      reads as light emanating from the crest into clean void -- dense just outside the lit edge,
#      gone (pure black) at the corners. 'a watcher in the dark.'
for y in range(H):
    for x in range(W):
        if in_mass(x, y): continue
        L = light(x, y)
