...
def body_region(x, y):
    ...
    if t < 0.25:
        ...
        halfw = 11.5 - 4.0 * ((t - 0.25) / 0.75)   # shoulders -> waist (modest taper, stays BROAD ~6.5)
...
def main():
    ...
    # P4b: sparse receding floor glints across the whole lower third -- depth language, not flat black.
    for y in range(36, H - 1):
        for x in range(W):
            ...
            if cur[0] != ' ':
                continue
            # dimmer + sparser with distance from center; converging toward the hero's base
            dx = abs(x - HERO_X)
            if random.Random(SEED + x * 13 + y * 7).random() < max(0.0, 0.28 - dx * 0.004):
