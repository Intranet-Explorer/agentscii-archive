...
SCLERA_RAMP = shade_ramp(15, 0, 5)    # (\u2588,\u2593,\u2592,\u2591,space) fg=15 bg=0: brightness faked by density
for cr in range(H):
    for px in range(W):
        u=(px-cx)/half_w
        ...
        lcur = max(0.03, min(1.0, l * (0.55 + 0.65*curve)))
         # local position along the brightness transition: 0 at lit pole, ~1 at dark rim.
        base = 1.0 - lcur
         # strong per-cell jitter so NO large contiguous patch shares one glyph -- this is
         # what breaks the flat region the gate flagged. Jitter shifts the density index by
         # up to ~2 levels between neighbours, so even the brightest zone shades across itself.
        jz = hsh(px, cr, 77)
        t = base + (jz - 0.5) * 0.9
        idx = int(max(0.0, min(1.0, t)) * 4 + 0.5)
        idx = max(0, min(4, idx))
        glyph, fg, bg = SCLERA_RAMP[idx]
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v58.ans","w").write(out)
print(f"orb v58 = v57 + shade_ramp(15,0) sclera pole dithering (flat-region gate fix: bright pole no longer collapses to a solid white patch -- glyph density carries brightness so even the lit pole shades across itself), 56 rows, {len(frame)} rows")
