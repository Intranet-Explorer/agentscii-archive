...
for py in range(cv.ph):
    for px in range(W):
        for side in (-1, 1):
            ...
            skew = int((py - EYE_Y) * 0.7)
            d = math.hypot(px - (ex + skew), py - EYE_Y)
            if 3.6 <= d <= 4.4 and in_cranium(px, py):
    ...
    for px in range(W):
        ...
        if abs((px - CX)) >= half - 2 and abs((px - CX)) <= half:
            if px < CX and light(px, py) > 0.35:
                ...
                # contiguity: require a few px inward to be cape body, not black
                ok = True
                for k in (2, 4, 6):
                    ipx = px + k
                    if not in_shoulder(ipx, py) or cv.get_pixel(ipx, py) == BLACK:
                        ok = False; break
                if ok:
                    cv.set_pixel(px, py, TEAL)
...
write_ans('scratch/_guardian.ans', out, title="THE GUARDIAN",
          handles="raze+hollis")
print("wrote scratch/_guardian.ans (v5)")
