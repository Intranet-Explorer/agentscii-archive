...
# v10 (raze): SMOOTH the fold field so bands read as soft cloth, not jagged lightning.
# Root cause of v9 jaggedness: independent per-column phase jitter (uncorrelated between
# neighbors) + high x-frequency made adjacent columns jump. Fix = smooth low-frequency
# column drift (correlated across x) + lower x-freq for wider folds + vertical blur so
# ridges/valleys are continuous in y.
_drift = [rng3.uniform(-0.5, 0.5) for _ in range(W)]
for _i in range(1, W - 1):
    for _k in range(3):
        _drift = [(_drift[max(0,i-1)] + _drift[i] + _drift[min(W-1,i+1)]) / 3.0 for i in range(W)]
_fold_field = [[0.0]*W for _ in range(cv.ph)]
for py in range(cv.ph):
    for px in range(W):
        f = math.sin(px * 0.32 + py * 0.06 + _drift[px] * 0.50)
        _fold_field[py][px] = f
for py in range(cv.ph):
    for px in range(W):
        a=_fold_field[max(0,py-1)][px]; b=_fold_field[py][px]; c=_fold_field[min(cv.ph-1,py+1)][px]
        _fold_field[py][px] = (a + 2*b + c) / 4.0
...
for py in range(cv.ph):
    for px in range(W):
        ...
        flat_top  = t < 0.20
         ...
         # coherent vertical folds from the smoothed _fold_field (v10): wide,
         # soft, vertically-continuous bands -- cloth drape, not jagged stripes.
        fold = _fold_field[py][px]                            # smoothed, precomputed (v10)
        amp = 0.16 + t * 0.24                                  # visible, grows gently to hem
        Lf = L + fold * amp
...
write_ans('scratch/_guardian.v10.ans', out, title="THE GUARDIAN",
          handles="raze+hollis")
print("wrote scratch/_guardian.v10.ans")
