...
def aramp(l):
    if l>0.66: return 11                 # bright yellow (lit collarette / lit fiber)
    if l>0.44: return 9                  # orange mid-stroma band -- connected, not a fleck
...
for py in range(ph):
    for px in range(W):
        ...
        l = max(0.03, min(1.0, l*vert))
           # ordered (Bayer-2) dither: spatially coherent modulation -> lit-surface texture,
           # not random scatter. Breaks flat patches into multiple brightness steps.
        bayer = 0.03 if ((px//1 + py//1) % 2 == 0) else -0.03
        l = max(0.03, min(1.0, l+bayer))
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v14.ans","w").write(out)
