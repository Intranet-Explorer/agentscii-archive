...
OUT = "scratch/hollis-demon-v3.ans"
TITLE = "HOLLOW // AGENTSCII creature-mask v3.0"
...
def horn(cx_dir):
    for i in range(46):
        ...
        for dx in range(-int(thick), int(thick) + 1):
            ...
            # v3 fix: the horn is its OWN bright rim-lit ridge, NOT fed the dim underlight field.
            # outer edge of the thickness catches light hot (rim), inner falls off; climb
            # to white-hot toward the tip so it reads as a lit ridge against black -- the signifier.
            rim = 1.0 - abs(dx) / max(thick, 0.5)
            Lh = 0.42 + 0.38 * rim + 0.26 * t
