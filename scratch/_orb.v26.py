...
# PASS 4c: COHERENT IRIS COLOR STRUCTURE -- v7's register (6 distinct subject colors).
#     Two structural elements, both coherent (NOT the scattered speckle that plagued v23):
#     (1) DIM-BROWN limbus band at the outer iris edge -- a solid dark-amber ring. Emitted as flat
#         brown cells so PASS 9 renders them as solid blocks (space+bg=3, i.e. 33;40 dim), matching
#         v7's brown-as-background structure. This is the 6th distinct color.
#     (2) SPARSE DIRECTIONAL bright-red(9) crypt accent on the limbus rim -- a few radiating ticks
#         aligned with the crypts, not a stroma flood (that was v23's defect). ~v7's register.
for py in range(ph):
    for px in range(W):
        if not in_opening(px,py): continue
        dx=px-cx; dy=py-cy; d=math.hypot(dx,dy)
        # (1) dim-brown limbus: outermost ~1.5px band of the iris
        if iris_r-1.6 <= d < iris_r+0.2:
            cv.set_pixel(px,py, 3)
# (2) sparse directional red accent on the rim, aligned with the NCRYPTS radii
for py in range(ph):
    for px in range(W):
        if not in_opening(px,py): continue
        dx=px-cx; dy=py-cy; d=math.hypot(dx,dy)
        if iris_r-1.2 <= d < iris_r+0.3:
            ang=math.atan2(dy,dx)
            tick = math.sin(ang*NCRYPTS + d*0.10)     # radiating ticks aligned with the crypts
            h=((px*53 + py*97) % 997)/997.0
            if tick > 0.6 and h < 0.5:                # directional, ~a dozen-to-twenty cells
                cv.set_pixel(px,py, 9)
...
for cr in range(H):
    for px in range(W):
        ...
        elif col==3:                                   # flat dim-brown limbus -> solid block, no bold
            cv.glyph_override[(cr,px)]=(' ',0,3)       # space+bg=3 = 33;40 dim brown (v7's structure)
        elif col == 11:                          # flat amber iris: dither with a brighter amber ink
            fg=11                  # hot amber ink; glyph density = the bridge
