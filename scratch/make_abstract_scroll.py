#!/usr/bin/env python3
# ABSTRACT SCROLL v1.1 -- AGENTSCII.
...
# fields into one interference system; credit card now reflects joint authorship.
# JOINT piece: hollis & raze. Output renamed to hollis-raze-abstract-scroll.ans. The STYLE.md ambition-ceiling piece for the ABSTRACT/GEOMETRIC tradition:
...
def make_abstract_title():
    ...
    p.put_text(19, CX - len("v1.1 -- five fields, one color cycle") // 2,
               "v1.1 -- five fields, one color cycle", 13)                    # magenta
...
# free sine-interference as base density, RADIAL's rings dictating hue by radius,
# GRIDFALL's perspective verticals fanning from a vanishing point overlaid as
# structure, and STREAM's comet heads seeded across the field. The four singles
...
def make_convergence():
    ...
    p0 = PHASE["p"]
    CXr, CYr = INNER_W / 2.0, H / 2.0
    VPY = 10.0                                 # vanishing point high & centered
    GLYPHS = "0123456789ABCDEF<>[]{}=+*/\\|#$%&@~^?"
    ...
    for y in range(H):
        for x in range(INNER_W):
            ...
            bright = 1.0 - dd / tl if in_comet else 0.0
                # compose: plasma density, lifted by ring shimmer + comet brightness
            dp = min(1.0, base * 0.6 + (shim / 3.0) * 0.4 + bright * 0.5)
            ch = RAMP[min(3, int(dp * 4))]
            if on_vline:
...
def make_credit():
    ...
    blocks = [
            ...
            ("05 CONVERGE   ", "four fields, one system ", "hollis"),
        ]
    y = 18
...
# ===========================================================================
# Local render_panel: wrap EVERY interior row in left/right double-line side borders
# so each content row is exactly 80 wide (house idiom -- the frame wraps the WHOLE
# scroll, not just top/bottom). Mirrors make_fractal_scroll.render_panel byte-for-byte.
# Left here LOCAL rather than changing scroll_lib.assemble() so no other builder that
# imports assemble() (mark_evolution, monitor_scroll x3, scroll_foundation/full/stage*)
# is silently re-framed by this one-piece fix.
ESC = "\x1b["
...
panels = [
    ...
    make_stream(),
    seam(8, index=5, phase0=PHASE["p"], label="CONVERGENCE"),
    make_convergence(),
    seam(8, index=6, phase0=PHASE["p"], label="CREDITS"),
    make_credit(),
]

# assemble with the local side-border-wrapped render_panel so every row is 80 wide.
lines = []
frame_top = sgr(105,40) + "\u2554" + sgr(104,40) + "\u2550" * (W - 2) + sgr(105,40) + "\u2557"
frame_bot = sgr(105,40) + "\u255a" + sgr(104,40) + "\u2550" * (W - 2) + sgr(105,40) + "\u255d"
lines.append(frame_top)
for p in panels:
    lines.extend(render_panel(p))
lines.append(frame_bot)
lines.append("\x1b[0m")                       # standalone reset tail (house convention)
open("hollis-raze-abstract-scroll.ans", "w", encoding="cp437").write("\n".join(lines))
print("wrote scratch/hollis-raze-abstract-scroll.ans (%d lines)" % len(lines))
