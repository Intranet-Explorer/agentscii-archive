    ...
    for py in range(ph):
        ...
        x_back = cx - math.sqrt(under)
         # front base: cranium arc on the face side
          # face-side base is FLATTER than a full semicircle (a profile's face
          # plane is near-vertical), so the head reads as a profile, not an orb.
        x_front_base = cx + math.sqrt(under) * 0.82
        t = (py - cy) / R               # ~-1 crown .. +1 chin, in head units
        front = 0.0                    # extra forward protrusion at this row
          # nose spike: a sharp triangular bump peaking at mid-face -- the single
          # strongest 'this is a face' cue in a profile silhouette.
        nose_t = 0.32
        if abs(t - nose_t) < 0.30:
            front += 0.62 * R * max(0.0, 1.0 - abs(t - nose_t) / 0.30)
          # chin: a forward jut near the bottom (t~0.85)
        if 0.72 < t < 1.0:
            front += 0.34 * R * max(0.0, 1.0 - abs(t - 0.86) / 0.24)
          # mouth recess: pull the front IN between nose and chin -- a concave dip
          # that separates nose from chin so the edge reads as a face, not a bump.
        if 0.50 < t < 0.72:
            front -= 0.30 * R
          # forehead: gentle forward slope above the brow (t~0.0-0.16)
        if -0.10 < t < 0.18:
            front += 0.12 * R * max(0.0, 1.0 - abs(t - 0.04) / 0.22)
