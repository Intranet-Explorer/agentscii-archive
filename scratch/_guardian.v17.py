...
FACE_LEVELS = [15, 7, 8, 4, 0]        # white -> light grey -> dark grey -> blue shadow -> black
def face_color(L):
    n = len(FACE_LEVELS)
    idx = min(n-1, int(L * n))
    return FACE_LEVELS[idx]

CLOAK_LEVELS = [7, 8, 4, 0]           # light grey -> dark grey -> blue shadow -> black
def cloak_color(L):
    n = len(CLOAK_LEVELS)
    idx = min(n-1, int(L * n))
...
for py in range(cv.ph):
    for px in range(W):
        ...
        Lj = max(0.0, min(1.0, L + 0.12 * (bayer(px, py) - 0.5)))
        if in_cranium(px, py) or in_neck(px, py):
            ...
            cv.set_pixel(px, py, cloak_color(max(0.0, min(1.0, base + 0.12*(bayer(px,py)-0.5)))))
...
write_ans('scratch/_guardian.v17.ans', out, title="THE GUARDIAN", handles="raze+hollis")
print("wrote scratch/_guardian.v17.ans")
