...
# --- v17 addition: cell-level density-glyph overlay so we can dither flat lit surfaces with ░▒▓
#      WITHOUT breaking half-block packing or the round silhouette. A dict keyed (cell_row, col) ->
#      glyph; render honors it for that cell only.
import re as _re2
_orig_render = HalfBlockCanvas.render
def _render_with_overlay(self):
    out=[]
    overlay=getattr(self,'glyph_overlay',{})
    for cell_row in range(self.h_cells):
        top_row=self.pixels[cell_row*2]; bot_row=self.pixels[cell_row*2+1]
        parts=[]; last_fg,last_bg=None,None
        for x in range(self.w):
            top,bot=top_row[x],bot_row[x]
            if (cell_row,x) in overlay:
                g=overlay[(cell_row,x)]
                # density glyph: fg and bg both = the lit color so it reads as that hue at reduced ink
                col=top
                ch,fg,bg=g,col,col
            elif top==bot:
                ch,fg,bg=' ',7,top
            ...
            if (fg,bg)!=(last_fg,last_bg):
                parts.append(_sgr(fg,bg)); last_fg,last_bg=fg,bg
            parts.append(ch)
        out.append(''.join(parts))
    return out
HalfBlockCanvas.render=_render_with_overlay
...
def aramp(l):
    ...
    if l>0.38: return 9                   # orange mid-stroma -- connected radial band
...
# --- PASS 7b: DENSITY DITHER on flat sclera surfaces (the gate's intended fix). Mark interior sclera
#      cells -- solid white/grey with a ▀ neighbour, i.e. on the lit surface not the void or a hard
#      edge -- as ░▒▓ density glyphs following the single light source. A lit surface shades across
#      itself via density variation; that's what makes a large grey/white patch read as a lit sphere
#      instead of a flat fill. The iris keeps its clean radial gradient; curves stay ▀ so it's round.
cv.glyph_overlay={}
RAMP=['\u2588','\u2593','\u2592','\u2591']   # full -> 75% -> 50% -> 25% ink
for cr in range(H):
    for px in range(W):
        top=cv.get_pixel(px,cr*2); bot=cv.get_pixel(px,cr*2+1)
        if top!=bot: continue                    # a ▀ curve cell -- leave it (keeps the silhouette round)
        col=top
        if col not in (7,8,15): continue          # only sclera greys/white get dithered
        # must have a ▀ neighbour (we're on the lit surface, not the flat void or a hard edge)
        has_curve=False
        for dx in (-1,0,1):
            if 0<=px+dx<W:
                t=cv.get_pixel(px+dx,cr*2); b=cv.get_pixel(px+dx,cr*2+1)
                if t!=b: has_curve=True; break
        if not has_curve: continue
        l=L(px,cr*2+0.5)
        dens=max(0.0,min(1.0,l*0.85+0.15))
        idx=int((1.0-dens)*3)%4
        ch=RAMP[idx]
        if ch!='\u2588':                        # only overlay when it actually changes the glyph
            cv.glyph_overlay[(cr,px)]=ch
...
out="\n".join(frame)+"\x1b[0m\n"
open("scratch/_orb.v17.ans","w").write(out)
